...
#import "TestastFatuular.h"//获取keychain中uuid用的自己的类
...
@implementation TyponessHugehood
...
//    ([[GamaFunction getSystemVersion] intValue]) >= 7 ? [GamaFunction getGamaUUID] : [GamaFunction getMacaddress];
    NSString * gamaUUid = [[TestastFatuular getCustomCfUUID] lowercaseString];
    return gamaUUid;
}

+ (NSString *)bundleSeedID
{
    NSString * gamaBundleSeedId=[TestastFatuular bundleSeedID];
    return gamaBundleSeedId;
}

+ (NSString *)getBundleName
{
    return [TyponessHugehood getProjectInfoPlist][@"CFBundleName"];
}

+ (NSString *)getBundleIdentifier
{
    return [TyponessHugehood getProjectInfoPlist][@"CFBundleIdentifier"];
}

+ (NSString *)getBundleShortVersionString
{
    return [TyponessHugehood getProjectInfoPlist][@"CFBundleShortVersionString"];
}

+ (NSString *)getBundleVersion
{
    return [TyponessHugehood getProjectInfoPlist][@"CFBundleVersion"];
}
...
+ (NSString*)getPreferredLanguage
{
    ...
    if ([TyponessHugehood getSystemVersion].intValue >= 9.0) {
...
+ (BOOL)checkValue:(id)value key:(NSString *)key andURLScheme:(NSString *)urlScheme andURLIdentifier:(NSString *)identifier
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [MeetHemisignate showAlertWithMessage:showTips];
    });
    return NO;
}
