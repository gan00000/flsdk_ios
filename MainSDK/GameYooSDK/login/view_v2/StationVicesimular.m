...
@implementation StationVicesimular
{
    GuberntyPyr *accountSDKTextFiledView;
    GuberntyPyr *passwordSDKTextFiledView;
    GuberntyPyr *passwordAgainSDKTextFiledView;
...
//        self.layer.masksToBounds = YES; //不设置这里会不生成圆角，原因查找中
        ...
        //title
//        mLoginTitleView = [[AboutiblePropertyible alloc] initViewWithTitle:@"註冊會員"];
...
//
//        //再次輸入密碼
//        passwordAgainSDKTextFiledView = [[GuberntyPyr alloc] initViewWithType:(SDKTextFiledView_Type_Password_Again)];
//
...
//
//
//        mPhoneView = [[ArticleesqueStrigosohise alloc] initView];
//
...
//        }];
//
//        getVfCodeBtn = [StenRoborious initBtnWithTitle2:@"獲取驗證碼" tag:kGetVfCodeActTag selector:@selector(registerViewBtnAction:) target:self];
//
...
//        [getVfCodeBtn setContentHuggingPriority:UILayoutPriorityRequired forAxis:UILayoutConstraintAxisHorizontal];
        ...
        regAccountBtn = [StenRoborious initBtnWithTitleText:@"text_confire_reg".localx fontSize:FS(17) textColor:[UIColor whiteColor] tag:kRegisterAccountActTag selector:@selector(registerViewBtnAction:) target:self];
...
//            NSString *newPassword = passwordAgainSDKTextFiledView.inputUITextField.text;
            ...
            if (![GenuosityFirst validPwd:pwd]) {
                return;
            }
          
            [AuousSituationic accountRegister:self.delegate view:self areaCode:@"" name:accountName password:pwd phoneNum:@"" vfCode:@""];
