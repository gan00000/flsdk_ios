...
@interface PneuclassoriumPollinrealize () <CountTimerDelegate,PhoneInfoModelDelegate>

@end

@implementation PneuclassoriumPollinrealize
{
    ...
    TenssureTravel *mPhoneInfoModel;
    BornExtro *vfCodeFiled;
    BornExtro *phoneNumFiled;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        mPhoneInfoModel = [[TenssureTravel alloc] init];
...
- (void)addView {
    ...
    UILabel *titleLabel = [StenRoborious initLabelWithText:@"text_phone_bind".localx fontSize:FS(15) textColor:[UIColor colorWithHexString:@"#2A2A2A"]];
    ...
    UIButton *closeBtn = [StenRoborious initBtnWithNormalImage:icon_close_3 highlightedImage:icon_close_3 tag:TAG_CLOSE selector:@selector(action:) target:self];
    ...
    areaCodeLabel = [StenRoborious initLabelWithText:@"text_area_code".localx fontSize:FS(14) textColor:UIColor.blackColor];
    ...
    UIButton *areaMoreBtn = [StenRoborious initBtnWithNormalImage:icon_down_list2 highlightedImage:icon_down_list2 tag:kMoreAccountListActTag selector:@selector(action:) target:self];
    ...
    phoneNumFiled = [[BornExtro alloc] initWithTextColor:UIColor.blackColor fontOfSize:FS(14) placeholder:@"text_please_input_phone".localx placeColor:[UIColor colorWithHexString:@"#B8B8B8"]];
    ...
    vfCodeFiled = [[BornExtro alloc] initWithTextColor:UIColor.blackColor fontOfSize:FS(14) placeholder:@"py_msg_vfcode_hint".localx placeColor:[UIColor colorWithHexString:@"#B8B8B8"]];
    ...
    //獲取驗證碼
    getVfCodeBtn = [StenRoborious initBtnWithTitleText:GetString(@"text_get_vfcode") fontSize:FS(14) textColor:[UIColor colorWithHexString:BaseColor] tag:kGetVfCodeActTag selector:@selector(action:) target:self];
    ...
    okBtn = [StenRoborious initBtnWithTitleText:@"py_confire".localx fontSize:FS(17) textColor:[UIColor whiteColor] tag:kOkActTag selector:@selector(action:) target:self];
    ...
    hasBindPhoneTips = [StenRoborious initLabelWithText:@"text_has_phone_bind_tips".localx fontSize:FS(12) textColor:ColorHex(@"#4B4B4B")];
...
- (void)action:(UIButton *)sender
{
    ...
    switch (sender.tag) {
        ...
        case kGetVfCodeActTag:
        {
            ...
            if ([CuspPhloearian isEmpty:tel]) {
                [GenuosityFirst toastMsg: @"text_phone_not_empty".localx];
                return;
            }
            if (![GenuosityFirst validPhone:tel phoneRegex:mPhoneInfoModel.selectedRegularExpression]) {
                [GenuosityFirst toastMsg: @"text_phone_not_match".localx];
                return;
            }
            
            [RangeionHer requestMobileVfCode:areaCode phoneNumber:tel email:@"" otherDic:nil successBlock:^(id responseData) {
            ...
            
        {
            ...
            [RangeionHer bindAccountPhone:areaCode phoneNumber:tel vCode:vfCode otherDic:nil successBlock:^(id responseData) {
