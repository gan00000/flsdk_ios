...
@interface VocallyCare()

@end

@implementation VocallyCare
{
...
//        make.height.mas_equalTo(VH(40));
    ...
    loginTabBtn = [StenRoborious initBtnWithTitleText:@"登入" fontSize:FS(24) textColor:[UIColor colorWithHexString:@"#848484"] tag:kLoginTabActTag selector:@selector(registerViewBtnAction:) target:self];
...
//        make.trailing.mas_equalTo(tabView).multipliedBy(0.5);
    ...
    regTabBtn = [StenRoborious initBtnWithTitleText:@"註冊" fontSize:FS(24) textColor:[UIColor colorWithHexString:@"#848484"] tag:kRegTabActTag selector:@selector(registerViewBtnAction:) target:self];
    ...
    //返回按钮
    UIButton *backBtn = [StenRoborious initBtnWithNormalImage:mw_back_icon highlightedImage:mw_back_icon tag:kBackBtnActTag selector:@selector(registerViewBtnAction:) target:self];
...
//    regTabBtn.backgroundColor = [UIColor whiteColor];
    ...
    self.mAccountLoginView = [[SinistritionCaptexplainine alloc] initView];
    ...
    self.mRegisterAccountView = [[LaughitorPressureose alloc] initView];
