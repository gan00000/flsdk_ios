...
@implementation ScyphanCardinency

{
    GuberntyPyr *oldPwdSDKTextFiledView;
...
- (instancetype)initView
{
    ...
    if (self) {
        ...
        //title
        mLoginTitleView = [[AboutiblePropertyible alloc] initViewWithTitle:GetString(@"text_change_pwd") hander:^(NSInteger) {
...
//            make.width.mas_equalTo(self);
        ...
        //旧密码
        oldPwdSDKTextFiledView = [[GuberntyPyr alloc] initViewWithType:(SDKTextFiledView_Type_Password_Old)];
        ...
        //確認
        UIButton *okBtn = [StenRoborious initBtnWithTitleText:GetString(@"text_confire_change") fontSize:FS(17) textColor:[UIColor whiteColor] tag:kOkActTag selector:@selector(registerViewBtnAction:) target:self];
...
-(void)changePassword
{
    ...
    if ([CuspPhloearian isEmpty:oldPwd]) {
    ...
    if (![GenuosityFirst validPwd:againPwd]) {
        return;
    }
    ...
    kWeakSelf
    [RangeionHer doChangePasswordWithUserName:userName andOldPassword:oldPwd andNewPassword:newPwd otherParamsDic:nil successBlock:^(id responseData) {
