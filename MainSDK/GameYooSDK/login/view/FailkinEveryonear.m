...
@interface FailkinEveryonear ()
@property (copy,nonatomic) void (^completer)(void);
@end

@implementation FailkinEveryonear
{
...
- (void)landspaceView {
    ...
    //title
    mLoginTitleView = [[AboutiblePropertyible alloc] initViewWithTitle:GetString(@"sdk_terms_title") hander:^(NSInteger param) {
...
//
//        //同意按鈕
//        agreeBtn = [StenRoborious initBtnWithNormalImage:@"btn_checkbox_uncheck.png" highlightedImage:nil tag:13 selector:@selector(action:) target:self];
...
//
//    //確認登入遊戲
//    UIButton *okButton = [PyrAmongproof initBtnWithType:(BUTTON_TYPE_OK) tag:kAccountLoginActTag selector:@selector(action:)  target:self];
...
+ (BOOL)openProvision{
    ...
    return ![FailkinEveryonear isAgreenProvision];
}
