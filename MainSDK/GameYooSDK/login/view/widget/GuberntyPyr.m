...
//    mUITextField.adjustsFontSizeToFitWidth = YES;//文字大小适配宽度大小n
    ...
    if (addMoreAccountBtn) {
        ...
        self.moreAccountBtn = [StenRoborious initBtnWithNormalImage:sdk_list_down highlightedImage:sdk_list_down selectedImageName:sdk_list_up tag:kMoreAccountListActTag selector:@selector(clickItemBtn:) target:self];
    ...
    if (showEye) {
        mUITextField.secureTextEntry = NO;
        eyeBtn = [StenRoborious initBtnWithNormalImage:fl_sdk_ky highlightedImage:fl_sdk_ky selectedImageName:fl_sdk_by tag:22 selector:@selector(eyeViewBtnAction:) target:self];
