...
@implementation BlastacleCoeldifferent
...
- (instancetype)initBtnViewWithType:(SDK_ICON_TITLE_BUTTON_TYPE)type tag:(NSUInteger)tag selector:(SEL)selector target:(id)target
{
    ...
    if (self) {
        ...
        //账号
        UIButton *mButton = [StenRoborious initBtnWithTitleText:nil fontSize:14 textColor:nil tag:tag selector:selector target:target];//[GenuosityFirst initBtnWithTag:tag selector:selector target:target];
