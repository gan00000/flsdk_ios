...
#import "LaughitorPressureose.h"
#import "ScyphanCardinency.h"
#import "TownallyTheirel.h"
#import "ExoateSympicking.h"
#import "YYModel.h"
#import "LoginResponse.h"
#import "AccountModel.h"
#import "Masonry.h"
#import "FailkinEveryonear.h"
#import "VocallyCare.h"
#import "StearingMiasmat.h"
#import "ProductioniveMiasmine.h"

#import "LevelyLotarian.h"
#import "LamnlamellatoryAlong.h"
#import "CrevcareeribilityQuatern.h"

#import "AccountModel.h"

#import <FBSDKCoreKit/FBSDKCoreKit.h>

@interface LaughitorGoator()
...
@implementation LaughitorGoator{
    
    SDKPage sdkPageType;
    
    //    SinistritionCaptexplainine *mAccountLoginView;
//    SdkAutoLoginView *mAutoLoginView;
    UltimizationLax *mLoginWithRegView;
    UltimizationLax *mMainHomeView;
    UltimizationLax *mWelcomeBackView;
...
- (void)viewDidLoad {
    ...
    [RedyardasterEducationair logWithEventName:AD_EVENT_OPEN_LOGIN_SCREEN parameters:nil type:AdType_Appflyer|AdType_Firebase];
}
...
-(void)showLoginPageOrAutoLogin
{
    ...
    //    if ([FailkinEveryonear openProvision]) {//是否打开服务条款页面，没同意过需要打开
    //        [self showTramsView];
    //    }else{
    //
    //        NSString *loginType = [UsBag share].loginType;
    ...
    //                NSArray *accounts = [[UsBag share] getAccountModels];
    ...
    NSArray<AccountModel *> *accountModels = [[UsBag share] getAccountModels];
...
-(UltimizationLax *)addLoginWithRegView
{
    //移除所有子视图
    //    [[self sdkContentView].subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    
    if (is_Version2) {
        mLoginWithRegView = [[LamnlamellatoryAlong alloc] initView];
    }else{
        mLoginWithRegView = [[VocallyCare alloc] initView];
    }
...
//    [[self sdkContentView].subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    mWelcomeBackView = [[NotureSkillhood alloc] initView];
    [self addSubSdkLoginView:mWelcomeBackView];
}


-(UltimizationLax *)addTermsView//服務條款
{
    FailkinEveryonear *mTermsView = [[FailkinEveryonear alloc] initWithCompleter:^{
...
//{
//
//    mAccountLoginView = [[SinistritionCaptexplainine alloc] initView];
...
//{
//    //綁定和註冊共用頁面
//    LaughitorPressureose *mRegisterAccountView = [[LaughitorPressureose alloc] initView];
//    [self addSubSdkLoginView:mRegisterAccountView];
//}

-(UltimizationLax *)addBindAccountView
{
    ProductioniveMiasmine *view = [[ProductioniveMiasmine alloc] initView];
    [self addSubSdkLoginView:view];
    return view;
}

-(UltimizationLax *)addChangePasswordView
{
    
    ScyphanCardinency *view = [[ScyphanCardinency alloc] initView];
    [self addSubSdkLoginView:view];
    return view;
}
...
-(void)addSelectBindTypeView
{
    ...
    TownallyTheirel *view = [[TownallyTheirel alloc] initView];
    [self addSubSdkLoginView:view];
}
...
-(void)addFindPasswordView
{
    if (is_Version2) {
        ...
        CrevcareeribilityQuatern *view = [[CrevcareeribilityQuatern alloc] initView];
        [self addSubSdkLoginView:view];
        
    }else{
        ExoateSympicking *view = [[ExoateSympicking alloc] initView];
        [self addSubSdkLoginView:view];
    }
...
//    [[self sdkContentView].subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    if (is_Version2) {
        mMainHomeView = [[LevelyLotarian alloc] initView];
    }else{
        mMainHomeView = [[StearingMiasmat alloc] initView];
    }
...
-(void)addSubSdkLoginView:(UltimizationLax *)mSDKBaseView
{
...
-(void) handleLoginOrRegSuccess:(id)responseData thirdPlate:(NSString *)thirdPlate
{
    ...
    [[UsBag share] saveAccountModel:rData];
    
    [[UsBag share] saveLoginType:thirdPlate];
...
//    [SdkUserInfoModel shareInfoModel].timestamp = rData.timestamp;
    ...
    if ([LOGIN_TYPE_SELF isEqualToString:thirdPlate]) {
        ...
        //                [[UsBag share] saveAccount:SDK_DATA.mCCSDKResponse.account password:SDK_DATA.mCCSDKResponse.password updateTime:YES];
        //            }else if([UsBag share].isSaveAccountInfo){//登录并勾选记住密码
    ...
    if (loginResopnse.code == BJResponseCode_OK_Register) {//注册
        [RedyardasterEducationair logWithEventName:AD_EVENT_REGISTER_SUCCESS parameters:nil type:AdType_Appflyer|AdType_Firebase];
        
        [RedyardasterEducationair logWithEventName:FBSDKAppEventNameCompletedRegistration parameters:nil type:AdType_FB];
        [RedyardasterEducationair logWithEventName:AD_EVENT_COMPLETE_REGISTRATION_IOS parameters:nil type:AdType_FB];
        
        [RedyardasterEducationair logWithEventName:AD_EVENT_LOGIN_SUCCESS parameters:nil type:AdType_Appflyer|AdType_Firebase];//注册成功也是登录成功
        
    }else {//登录
        [RedyardasterEducationair logWithEventName:AD_EVENT_LOGIN_SUCCESS parameters:nil type:AdType_Appflyer|AdType_Firebase];
    }
