...
@interface NearEnvironmentitude ()
@property (nonatomic, copy) NSString *m_stringsName;
@property (nonatomic, strong) NSBundle *m_stringsBundle;
@end

@implementation NearEnvironmentitude

static NearEnvironmentitude * coreReader;
static dispatch_once_t onceToken;


+ (NearEnvironmentitude *)reader
{
    
    dispatch_once(&onceToken,^{
        coreReader = [[NearEnvironmentitude alloc] init];
...
-(NSString *) getGameLanguage
{
    if ([self isMoreLanguage]) {
        return [TyponessHugehood getServerLanguage];
    }
    if ([CuspPhloearian isNotEmpty:[self getStringForKey:@"gameLanguage"]]) {
