...
//#define winWidth        [TyponessHugehood getCurrentScreenFrame].size.width//[UIScreen mainScreen].bounds.size.width
//#define winHeight       [TyponessHugehood getCurrentScreenFrame].size.height//[UIScreen mainScreen].bounds.size.height
...
+ (BOOL)validPwd:(NSString *)pwd
{
    ...
    if (pwd.length < 6) {
        [GenuosityFirst toastMsg:GetString(@"text_pwd_format")];
        return NO;
    }
    return YES;
...
//        baseView = [TyponessHugehood getCurrentWindow];
    }
...
//        toastLabel.lineBreakMode = NSLineBreakByWordWrapping;
        ...
        CGSize tempStringSize = [GenuosityFirst calculateSizeOfLabel:toastLabel];
...
{
    return [DifficulteousFewably getEncryptStringFromString:aString WithKey:kEncryKey iv:kEncryIv];
}

+ (NSString *)loginDecryptFromString:(NSString *)aString
{
    return [DifficulteousFewably getDecryptStringFromString:aString withKey:kEncryKey iv:kEncryIv];
}
