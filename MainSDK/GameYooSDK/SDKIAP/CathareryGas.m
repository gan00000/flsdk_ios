...
@interface CathareryGas ()<SKProductsRequestDelegate,SKPaymentTransactionObserver>
...
@implementation CathareryGas

+ (instancetype)shareManager
{
    static dispatch_once_t onceToken;
    static CathareryGas *manager = nil;
    dispatch_once(&onceToken, ^{
        manager = [[CathareryGas alloc]init];
        [manager initManager];
    });
    return manager;
}
...
- (void)checkOrderStatus
{
    ...
    if (orderInfoDic != nil) {
        ...
        for (id keyObj in orderInfoDic) {
            ...
            [RangeionHer paymentWithTransactionId:transactionId receiptData:receiptData orderId:orderId gameInfo:SDK_DATA.gameUserModel accountModel:SDK_DATA.mLoginResponse.data otherParamsDic:nil successBlock:^(id responseData) {
...
-(void)startPayWithProductId:(NSString *)productId cpOrderId:(NSString *)cpOrderId extra:(NSString *)extra gameInfo:(DorspoorencyDesdata*)gameUserModel accountModel:(AccountModel*) accountModel payStatusBlock:(PayStatusBlock)payStatusBlock
{
    ...
    //[GenuosityFirst showLoadingAtView:nil];
    [RangeionHer createOrderWithproductId:productId cpOrderId:cpOrderId extra:extra gameInfo:gameUserModel accountModel:accountModel otherParamsDic:nil successBlock:^(id responseData) {
...
//    }
    //設置用戶信息
    if ([TyponessHugehood getSystemVersion].intValue>7){
...
-(void)handleTransaction:(SKPaymentTransaction *)transaction
{
    ...
    if ([CuspPhloearian isEmpty:transactionId]) {
...
//    NSString *receiptString = [receiptData base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
    ...
    NSString *receiptString = [TyponessHugehood encode:(uint8_t *)receiptData.bytes length:receiptData.length];
...
//    在開發過程中，無法直接關聯 transaction 與 orderID 之間聯絡，雖然有一個 applicationUserName 欄位，可以儲存一個資訊。但是這個欄位是不是 100%靠譜，在某些情況下會丟失儲存的資料。
    ...
    [GenuosityFirst showLoadingAtView:nil];
    [RangeionHer paymentWithTransactionId:transactionId receiptData:receiptString orderId:parameterStr gameInfo:SDK_DATA.gameUserModel accountModel:SDK_DATA.mLoginResponse.data otherParamsDic:nil successBlock:^(id responseData) {
...
- (void)finishPayWithStatus:(BOOL)status msg:(NSString *)msg
{
    ...
    if (status) {
        ...
        if ([CuspPhloearian isNotEmpty:msg]) {
            [MeetHemisignate showAlertWithMessage:msg];
        }
        if (self.payStatusBlock) {
