...
//                                                                   object:nil];
//            }
//        } andButtonTitles:SDKConReaderGetLocalizedString(GAMA_FACEBOOK_BIND_CONFIRM),SDKConReaderGetLocalizedString(GAMA_FACEBOOK_BIND_CANCEL), nil];
    }
}
...
+(void)_facebookFreeBindingWithFacebookID:(NSString *)facebookID UserId:(NSString *)userID
{
    ...
    BOOL isKoreaRegion = [SDKConReaderGetString(@"current_Select_Region") hasPrefix:@"server_KR"] ? YES : NO ;
    if (isKoreaRegion) {
        // 开始进行FB绑定
