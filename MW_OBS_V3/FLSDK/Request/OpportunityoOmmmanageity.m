...
**/
@interface OpportunityoOmmmanageity ()
...
@end
@implementation OpportunityoOmmmanageity
+ (instancetype)sharedInstance {
    static OpportunityoOmmmanageity *instance;
...
**/
    ...
    dispatch_once(&onceToken, ^{
        instance = [[OpportunityoOmmmanageity alloc] init];
...
+ (void)getRequestWithFunctionPath:(NSString *)path
    ...
    [[OpportunityoOmmmanageity sharedInstance].httpEngine getRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
...
**/
    ...
    [OdorSever showLoadingAtView:nil];
    [[OpportunityoOmmmanageity sharedInstance].httpEngine postRequestWithFunctionPath:path params:allParams successBlock:^(NSURLSessionDataTask *task, id responseData) {
...
+ (void)fileUploadWithFunctionPath:(NSString *)functionPath
                     ...
                     progressBlock:(BJHTTPProgressBlock)progressBlock
                      successBlock:(BJServiceSuccessBlock)successBlock
                        errorBlock:(BJServiceErrorBlock)errorBlock {
    [[OpportunityoOmmmanageity sharedInstance].httpEngine fileUploadWithFunctionPath:functionPath params:params fileData:fileData fileName:fileName mimeType:mimeType progressBlock:^(float progress) {
        if (progressBlock) {
...
// 《Flightless Bird, American Mouth》是由Iron & Wine乐队演唱的一首独立民谣,Iron & Wine 这个一个人的乐队，却用让人赞叹的旋律和歌词带给听众一首清新雅致而又静生澎湃田园式的美国
                       successBlock:(BJServiceSuccessBlock)successBlock
                         errorBlock:(BJServiceErrorBlock)errorBlock {
    [[OpportunityoOmmmanageity sharedInstance].httpEngine imageUploadWithFunctionPath:functionPath params:params imageData:imageData imageName:imageName progressBlock:^(float progress) {
        if (progressBlock) {
            progressBlock(progress);
        }
    } successBlock:^(NSURLSessionDataTask *task, id responseData) {
