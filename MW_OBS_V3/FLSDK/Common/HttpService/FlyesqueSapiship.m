...
#import "FlyesqueSapiship.h"
#import "AFNetworking.h"
#import "ExtraressApertless.h"
@interface FlyesqueSapiship ()
@property (nonatomic, strong) AFHTTPSessionManager *sessionManager;
@end
@implementation FlyesqueSapiship
...
/**
  cellent evidence that self-imposed cash penalties motivate success. You can make a bet with a friend that you'll stick to your New Year's resolution. The logic for why this works is simple. Incentives change our decisions, and penalties are even more motivating than rewards 
**/
    return self;
}
...
/**
   remember when to act. Be sure t 
**/
{
...
/**
  ercome your racing mind and allow you to fall asleep."
...
//I miss the way the sun shines at your face and the way the cold breeze of the morning surprises us.
            if (successBlock) {
                successBlock(task, responseObject);
//意，也许需要一小时，但是这段时间不要醒着躺在床上。”
//
//Why is that so important? Because lying in bed awake can form an association in your brain that can lead to chronic insomnia, Grandner explained. Instead of being 
            }
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            if (errorBlock) {
                errorBlock(task, error);
...
{
// decline a proffered lunch meeting.
//
...
/**
  月与卿。日为朝，月为暮，卿为朝朝暮暮。
Even if from another world, still 
**/
- (void)postJsonRequestWithFunctionPath:(NSString *)path
    ...
    @try {
        [self.sessionManager POST:path parameters:params headers:@{@"Content-Type" : @"application/json"} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            if (successBlock) {
                successBlock(task, responseObject);
//t know how long I've waited for you. And so the lion fell in love with the lamb. What a stupid lamb. What a sick, masochistic lion.
...
/**
  a marathon or write a book, you'd be wise to s 
**/
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            if (errorBlock) {
                errorBlock(task, error);
...
//n our lives, we are dropped, crumpled, and ground into the dirt by the decisions we make and the circumstances that come our way. We feel as though we a
