...
/**
  ere is evidence that coachin 
**/
#import "SeeizeMacismallary.h"
#import "UIView+GonyticSocialance.h"
#import "AperthandBalowise.h"
//有乐趣，就不太可能坚持下去。但研究发现如果你从锻炼或学习中获得乐趣，就会坚持得更久。而最终，这往往是实现新年计划的最重要因素。
//
//
//
//One way to make pursuing a goal that norma
...
A well-known speaker started off his seminar by holding up a $20 bill. In the room of 200, he asked, “Who wo 
**/
@interface FlectiblePantator()
@end
@implementation FlectiblePantator
{
    UIButton *guestLoginBtn;
    UIButton *checkBoxTermsBtn;
}
//ions so there is a hook to get you to the library.
//
...
**/
    ...
    return self;
//d.Sun, Moon and you. Sun for morning, Moon for night, and you forever.
...
//hinking needs to be changed, Grander said.
//
//格兰德纳说，这种想法必须改变。
//
//"Do
    UIImageView *logoIV = [BedeurFail initImageViewWithImage:mw_logo];
    logoIV.hidden = YES;
...
/**
  e ground and started to grind it into t 
**/
...
/**
   in the towel. Researchers call this the "what the hell effect." Here's what it looks like: You planned to get to bed early every night but couldn't resist staying up late one Friday to watch an extra episode of "Succession." 
**/
        }
        make.centerX.mas_equalTo(contentView);
    }];
    guestLoginBtn = [BedeurFail initBtnWithTitleText:@"" fontSize:FS(17) textColor:[UIColor whiteColor] tag:guestLoginActTag selector:@selector(registerViewBtnAction:) target:self];
//ht of poor sleep.
//
...
/**
  s to express how much I miss you
...
// later and find out you haven't followed through.
//
//一个简单的方法是把你的目标告诉一些人，这样，如果他们后来发现你没有坚持下去，你会感到羞愧。
//
//
//
//A steeper penalty than shame, however, is putting cold hard cash on the table, and there is excellent evidence that self-imposed cash penalties motivate success. You can make a bet with a friend that you'll stick to your Ne
    }];
    UIButton *guestIconBtn = [BedeurFail initBtnWithNormalImage:guse_login_bg highlightedImage:nil tag:guestLoginActTag selector:@selector(registerViewBtnAction:) target:self];
...
/**
  umpled or finely creased you are still priceless to those who love you. The worth of our lives comes, not in what we do or who we know, but by Who We Are.”
...
/**
   to your New Year's resolution. The logic for why this works is sim 
**/
...
//ay sometimes stand in the way of your dreams, remember that your destiny is hiding behind them. Accept the fact
        if (!SDK_DATA.mConfigModel.appleLogin || SDK_DATA.mConfigModel.appPassCheck) {

/**
  ree cards each week, you can get better results than by setting either tough or easy goals without wiggle room, research has revealed.

幸运的是，有一种方法可以避免这种宿命。研究表明，设定困难的目标（比如每晚10点入睡），但每周给自己一两张豁免卡，就可以比设定 
**/
...
// this?” He dropped it on the ground and sta
...
/**
  u toss and turn and wake up tired.

为什么这条法则如此重要呢？格兰德纳解释道，因为醒着躺在床上会在大脑中形成关联，从而导致长期失眠。这样一来，床就不再是你安眠的舒适窝，而成为你辗转反侧心生焦虑并在疲惫中醒来的地方。

Establishing that positive relationship between the bed and sleep can be beneficial on nights where your schedule has to be erratic due to work or travel, Grandner a 
**/
            }];
        }
    }
...
/**
  u might not otherwise.

奇怪的是，有证据表明，指导有共同目标的朋友也能提高你的成功率。当你想给别人一些如何实现 
**/
...
//sts your self-confidence. It also forces you to be introspective about what works in ways you might not otherwise.
//
...
//每天在固定时间起床和上床
//
//Melatonin is a hormone produced by the bod
    }];

/**
  效。如果你的目标是锻炼身体，你会认为高强度的锻炼将能让你快速取得成效。如果你想学好 
**/
    UILabel *hasAccountLabel = [BedeurFail initLabelWithText:@"已有帳號？ " fontSize:FS(12) textColor:[UIColor whiteColor]];
    [hasAccountContent addSubview:hasAccountLabel];

/**
  ind them. Accept the fact that not everyone is going to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, 
**/
...
/**
   high and dry because you'll neglect an even more important part of the equation: whether you enjoy the act of goal pursuit.
...
If it's not fun to exercise or study, you're unlikely to keep at it. But if you get pleasure from your workouts or study sessions, res 
**/
    }];
...
//
//
//One easy way to do this is by telling a few people 
...
/**
  单。激励措施会改变我们的决定，而惩罚甚至比奖励让我们更有动力。
...
Most of us strive for efficiency when it comes to achieving our goals. If you want to get fit, y 
**/
        make.height.mas_equalTo(1);
//ne Friday to watch an extra episode of "Succession." After that, your early-to-bed plans went out the window because "what the hell," you'd already failed.
//
//如果你完全违背了自己的新年计划，你可能本能地认输并放弃。研究人员称之为“去他的效应”。就比如这样：你计划每晚早睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了，因为“去他的”反正你已经失败了。
//
//
//
//Happily, there is a way to dodge this fate. By setting tough goals (like a 10 pm bedtime every night) but gi
        make.width.mas_equalTo(VW(26));
...
//ship between the bed and sleep can be beneficial on nights where your schedule has to be erratic due to work or travel, Grandner added
        make.centerX.mas_equalTo(contentView);

/**
  he star that holds your destiny, the one that forever twinkles within your heart. Take advantage of precious opportunities while they still sparkle before you. Always believe that your ultimate goal is attainable as long as you commit yourself to it.
...
//like: You planned to get to bed early every night but couldn't resist staying up late one Friday to watch an extra episode of "Su
    }];
    CGFloat btn_w = VW(28);
    CGFloat btn_h = btn_w;
...
**/
    ...
    if (SDK_DATA.mConfigModel.appPassCheck) {
        loginBtnDatas = [OdorSever getShowBtnDatas:SDK_DATA.mConfigModel appleBtn:YES guestBtn:NO];
    }else{
        loginBtnDatas = [OdorSever getShowBtnDatas:SDK_DATA.mConfigModel appleBtn:NO guestBtn:NO];
...
Most adults need between seven and eight hours of sleep to be fully rested, according to t 
**/
    }
...
//ble lesson. No matter what I did to the money, you still wanted it because it did not decrease in value. It was still worth $20."
//
//
//
...
/**
  erformance. If your New Year's resolution is to run a marathon or write a book, you'd be wise to start hanging around friends who've made it to the finish line (literally or figu 
**/
        }else{
            ...
            btnView = mBtn;
// of "Succession." After that, your early-to-bed plans went out the window because "what the hell," you'd already failed.
//
//如果你完全违背了自己的新年计划，你可能本能地认输并放弃。研究人员称之为“去他的效应”。就比如这样：你计划每晚早睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了
        }
        if (btnView) {

/**
   love with the lamb. What a stupid lamb. What a sick, ma 
**/
...
//
//
//“Dirty or clean, crumpled or finely creased you are still priceless to those who love you. The worth of our lives comes, not in what we do or
...
//tudy sessions, research has found you'll persist longer. And in the end, that's what often matters most to achieving a New Year's resolution.
//
...
我的心很疼，我很想念你。
Catch the star that holds your destiny, the one that forever twinkles within your heart. Take advantage of precious opportunities while they still sparkle before you. Always believe that your ultimate goal is attainable as long as y 
**/
...
/**
  etting yourself watch your favorite TV show at the gym so you'll start looking forward to workout 
**/
    }];
...
/**
  Because lying in bed awake can form an association in your brain that can lead to chronic insomnia, Grandner explained. Instead of being a restful spot where you peacefully fall asleep, your bed becomes an anxious place where you 
**/
...
在完成目标的过程中，大多数人都力求高效。如果你的目标是锻炼身体，你会认为高强度的锻炼将能让你快速取得成效。如果你想学好一门功课，你会认为长时间、不被打扰的学习很重要。但研究表明，专注于效率会让你被困在原地，因为你会忽 
**/
         };
...
// marathon or write a book, you'd be wise to start 
...
/**
  hologist and sleep expert Michael Grandner. So if your bedtime and wake up time change from day to day or o 
**/
            }
            break;

/**
  re even more motivating than rewards.
...
1. Make a schedule, and stick to i 
**/
            if (![self checkAgreeTerm]) {
                return;
            }
...
//
//
//Just as cues tell Broadway stars when to step onto the stage, research has shown that adding a cue 
            if (![self checkAgreeTerm]) {
                return;
            }
...
Take pride in your accomplishments, as they are stepping stones to your dreams. Understand that you may make mistakes, but don’t let them discourage you. Value your capab 
**/
            if (![self checkAgreeTerm]) {
                return;
            }
...
**/
            if (![self checkAgreeTerm]) {
                return;
// has the power to help overcome your racing mind and allow you to fall asleep."
//
//他说：“比如说你需要提早上床睡觉。床将能帮助你安抚纷繁的思绪，让你酣然入睡。”
//
//3. Change your attitude about sleep
...
//改变对睡眠的心态
//
//Many people view sleeping as the final thing they have to do in a jam-packed day, worth delaying to catch up o
            }
...
**/
            break;
//ren’t friends, not that I didn’t wanna be.
//我只是说如果我们不是朋友会好很多，但是那不代表我不想和你交朋友。
...
/**
   you need to go to bed extra early," he said. "The bed now has the power to help overcome your racing mind and allow you to fall asleep."
...
//我对你的爱，至死不渝。
//Now I'm afraid. I'm not afraid of you… I'm only afraid of losing you. Like 
-(BOOL)checkAgreeTerm
{
    if (checkBoxTermsBtn.selected) {
        return YES;
