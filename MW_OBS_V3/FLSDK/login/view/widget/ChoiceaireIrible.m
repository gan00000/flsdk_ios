...
@implementation ChoiceaireIrible
// your memory when it's opportune and generates guilt if you flake out. Detailed planning can a
{
...
/**
  re's what it looks like: You planned to get to bed early every night but couldn't resist staying up late one Friday to watch an extra episode of "Success 
**/
...
//n make a bet with a friend tha
...
-(void)setPwdFiledView:(BOOL)enable{
    ...
    if (enable) {
        ...
        mUITextField.text = @"";
        eyeBtn.selected = YES;
//n the room of 200, he asked, “Who would like this $20 bill?”
//
//
//
...
// you. Like you're going to disappear…
//现在我害
...
/**
  t dying in the place of someone I love,seems lik 
**/
    BOOL showEye = NO;
...
//ur hopes everything you’ve got and yo
    switch (type) {
...
/**
  d to chronic insomnia, Grandner explained. Instead of being a restful spot where you peacefully fall asleep, your bed becomes an anxious place where you toss and turn and wake up tired.

为什么这条法则如此重要呢？格兰德纳解释道，因为醒着躺在床上会在大脑中形成关联，从而导致长期失眠。这样一来，床就不再是你安眠的舒适窝，而成为你辗转反侧心生焦虑 
**/
...
//图片
//
// 《Flightless Bird, American Mouth》是由Iron & Wine乐队演唱的一首独立民谣,Iron & Wine 这个一个人的乐队，却用让人赞叹
            break;
        case SDKTextFiledView_Type_Password:
//起来只是想法上的一个小小改变，但却很重要。
//
...
If your New Year's resolution is to meditate five days each week, a plan like "I'll meditate on weekd 
**/
...
/**
  h day, backtiming eight hours would require a bedtime of 11 pm.

美国疾病控制与预防中心指出，大多数成人需要7到8小时睡眠时间才能得到充分休息。因此如果一个人每天要在早上7点起床，倒推8个小时睡眠时间应该是晚上11点。

"Now you know when you have to stop and get ready to go to bed whether you're done or not," Grander said. "The problem is we don't stop, and we don't disconnect. And that's to our detriment and it makes 
**/
        default:
            break;
    }
...
//You're in here because of me.
//你在这里，是因为有我。
//Nobody's ever loved anybody as much as 
...
//我从来没有想过自己会怎么死。但是死在自己爱的人的怀里看起来
        make.height.mas_equalTo(VH(21));

/**
  r brightest dreams come true. Give your hopes everything you’ve got and you will catch  
**/
...
//ne way to make pursuing a goal that normally feels like a chore more fun is to combine it 
    }];
//自己爱的人的怀里看起来是一个不错的选择。
//Your mood swings are kind of giving me a whiplash.
//你的态度忽冷忽热让我难受。
//I only said it’d be better if we weren’t friends, not that I didn’t wanna 
...
“Dirty or clean, crumpled or finely creased you are stil 
**/
    if (addMoreAccountBtn || showEye) {
        trailing_offset = -VW(30);

/**
  We feel as though we are wo 
**/
    }
    mUITextField = [[UITextField alloc] init];

/**
  lf watch your favorite TV show at the gym so you'll start looking forwa 
**/
    self.inputUITextField = mUITextField;
//es -- so if you plan to meditate during lunch, you'll be sure to decline a proffered lunch meeting.
//
//提前想好你会在何时何
    if (mUIKeyboardType) {

/**
  ry night) but giving yourself one or two get-out-of-jail-free cards each week, you can get better results than by setting either tough or easy  
**/
...
//e special ——don't ever forget it.”
//
//
//
//“你是与众不同的，永远不要忘记这一点!”
...
/**
  。如果你的上床和起床时间每天都不一样或者一到周末就改变，身体就无法预测你的睡眠节奏，也就不知道如何作出反应。
...
A well-known speaker started off his seminar by holding up a $20 bill. In the room of 200, he asked, “Who w 
**/
...
// become your greatest weakness ,then going all you can do to protect each other.
//爱就是让对方成为自己最大的弱点，然后再用尽全力守护彼此。
//Is it not enough just to have a long and happy life with me?
//只是跟我共度幸福美满的漫长一生，难道还不够吗？
//Only for making me want to you badly. I still don't know if I can control myself.
//就是因为发觉自己多么渴望想得到你，我到现在还不知道是否控制得了自己。
//When life offers you a dream so far beyond any of your expectations, it’s not r
...
/**
  l myself.
就是因为发觉自己多么渴望想得到你，我到现在还不知道是否控制得了自己。
When life offers you a dream so far beyond any of your expec 
**/
        self.moreAccountBtn = [BedeurFail initBtnWithNormalImage:sdk_list_down highlightedImage:sdk_list_down selectedImageName:sdk_list_up tag:kMoreAccountListActTag selector:@selector(clickItemBtn:) target:self];
...
**/
    ...
    if (showEye) {
        mUITextField.secureTextEntry = NO;
        eyeBtn = [BedeurFail initBtnWithNormalImage:fl_sdk_ky highlightedImage:fl_sdk_ky selectedImageName:fl_sdk_by tag:22 selector:@selector(eyeViewBtnAction:) target:self];
        eyeBtn.selected = YES;

/**
  jogs your memory when it's opportune and generates guilt if you flake out. Detailed planning can also help you anticipate and dodge obstacles -- so if yo 
**/
        mUITextField.secureTextEntry = YES;
...
/**
  till wanted it because it did not decrease in value. It was still worth $20."
...
/**
  introspective about what works in ways you might not otherwise.
...
/**
  ep as the amount of time you have left in your day," he advised. "See your sleep as the amount of time 
**/
{
...
/**
  ur face and the way the cold breeze of the morning surprises us.

我想念阳光照在你的脸上，早晨的寒风让我们 
**/
    switch (type) {
...
/**
  t with a friend that you'll stick to your New Year's resolution. The logic for why this works is simple. Incentives change our decisions, and penalties are even more motivating than rewards.
...
/**
  ugh just to have a long and happy life with me?
只是跟我共度幸福美满的漫长一生，难道还不够吗？
Only for making me want to you badly. I still don't know if I can control myself.
就是因为发觉自己多么渴望想得到你，我到现在还不知道是否控制得了自己。
...
/**
  ur plan helps you remember when to act. Be sure to detail when and where you'll follow through.

正如出场提示告诉百老汇明星何时登台一样，研究表明，在你的计划中加入信息提示，能帮助你记住何时行动。一定要详细说明你将在何时何地做什么。



If your New Year's resolution is to meditate five days each week, a plan like "I'll meditate on weekdays" would be too vague. But a cue-based plan like "I'll meditate at the office 
**/
            break;
    }
    self.lableIconImageView = [[UIImageView alloc] initWithImage:[UIImage res_imageNamed:iconName]];
...
/**
  r dreams. Understand that you may make mistakes, but don’t let them discourage yo 
**/
        make.height.mas_equalTo(VH(21));
//his fate. By setting tough goals (like a 10 pm bedtime every night) but gi
        make.width.mas_equalTo(self.lableIconImageView.mas_height);
    }];
    UIView *lineView1 = [[UIView alloc] init];
    lineView1.backgroundColor = [UIColor colorWithHexString:understand_establishaster];
...
You're in here because of me.
你在这里，是因为有我。
Nobody's ever loved anyb 
**/
        make.leading.mas_equalTo(self.lableIconImageView.mas_trailing).mas_offset(VW(8));
//但是那不代表我不想和你交朋友。
//He unleashed the full, devastating powe
        make.centerY.mas_equalTo(self);
        make.height.mas_equalTo(VH(14));
    }];
    int trailing_offset = -10;
...
/**
  d. "The bed now has the power to help overcome your racing mind and allow you to fall asleep."
...
"The best sleep tip you can ever give somebody  
**/
    }
    int placeHolderTextSize = FS(14);
    NSAttributedString *attrStr = [[NSAttributedString alloc] initWithString:placeholderText
    ...
    [mUITextField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(lineView1.mas_trailing).mas_offset(VW(8));
        make.top.mas_equalTo(self).offset(2);
//, and we don't disconnect. And that's to our detriment and it makes the next day more stressful."
//
//There are not enough ways to express how much I miss you
//
//没有足够的方式能表达我有多想念你
...
//t hanging around friends who've made it to the finish line (literally or figuratively) and can show you how it's done. You'll pick up a bit just by spending time together because you'll be inclined to conform 
    if (addMoreAccountBtn) {
...
/**
  t research has shown that focusing on efficiency can leave you high and dry because you'll neglect an even more important part of the equation: whether you enjoy the act of goal pursuit.

在完成目标的过程中，大多数人都力求高效。如果你的目标是锻炼身体，你会认为高强度的锻炼将能让你快速取得成效。如果你想学好一门功课，你会认为长时间、不被打扰 
**/
        [self addSubview:self.moreAccountBtn];

/**
  e amount of time you need in order to set yourself up for a productive tomorrow."

他建议：“不要将睡眠时间看成你一天中剩下的时间，而是将其看成为了高 
**/
        [self.moreAccountBtn mas_makeConstraints:^(MASConstraintMaker *make) {
...
"The best sleep tip you can ever give somebody is get up -- don't lay in bed awake but not 
**/
            make.centerY.mas_equalTo(self);
            make.height.mas_equalTo(self.mas_height);

/**
  人都力求高效。如果你的目标是锻炼身体，你会认为高强度的锻炼将能让你快速取得成效。如果你想学好一门功课，你会认为长时间、不被打扰的学习很重要。但研究表明，专注于效率会让你被困在原地，因为你会忽略更重要的事：你是否享受追求目标的过程。



If it's not fun to exercise or study, you're unlikely to keep at it. But if you get pleasure from your workouts or study sessions, research has found yo 
**/
            make.width.mas_equalTo(VW(17));
...
/**
  I do this?” He dropped it on the ground and started to grind it into the floor with his shoe. He picked it up, n 
**/
    if (showEye) {
//riends, you have all learned a very valuable lesson. No matter what I did to the money, you still wanted it because it did not decreas
        mUITextField.secureTextEntry = NO;
        eyeBtn = [BedeurFail initBtnWithNormalImage:fl_sdk_ky highlightedImage:fl_sdk_ky selectedImageName:fl_sdk_by tag:22 selector:@selector(eyeViewBtnAction:) target:self];
        eyeBtn.selected = YES;
        mUITextField.secureTextEntry = YES;
...
/**
  and that you may make mistakes, but don’t let them discourage you. Value your capabilities and talents for they are what make you truly unique. The greatest gifts in life are not purchased, but acquired through hard work a 
**/
            make.height.mas_equalTo(self.mas_height);
...
He proceeded to crumple the 20 dollar note up. He then asked, 
**/
            make.width.mas_equalTo(VW(17));
//调节。随着黑夜降临，褪黑素的水平就会升高，暗示身体该睡觉了。光线会阻止身体分泌褪黑素，所以白天来临时褪黑素水平就会下降，让你准备好迎接新的一天。
//
//To work properly, the release of this hormone needs to occur at regular times, said clinical psychologist and sleep expert Michael Grandner. So if your bedtime and wake up time change from day to day or on weekends, he said, your sleep rhy
        }];
    }
//
//
...
/**
  u have to stop and get ready to go to bed whether you're 
**/
        make.bottom.mas_equalTo(self);
        make.height.mas_equalTo(1);
    }];
...
**/
        lineView1.hidden = YES;
        lineView2.hidden = YES;
//epping stones to your dreams. Understand that you may make mistakes, but don’t let them discourage y
    }
    if (self.inputUITextField) {
//or Disease Control and Prevention. So if a person needed to rise at 7 am each day, backtiming eight hours would require a bedtime of 11 pm.
//
//美国疾病控制与预防中心指出，大多数成人需要7到8小时睡眠时间才能得到充分休息。因此如果一个人每天要在早上7点起床，倒推8个小时睡眠时间应该是晚上11点。
//
//"Now you know when you have to stop and get ready to go to bed whether you're done or not," Grander said. "The problem is we don't stop, and we don't di
        [self.inputUITextField addTarget:self action:@selector(textChanged:) forControlEvents:(UIControlEventEditingChanged)];
    }
}
...
/**
  u just need five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."
...
/**
   Value your capabilities and talents for they are what make you truly unique. The greatest gifts i 
**/
        mUITextField.secureTextEntry = YES;
        mUITextField.text = tempPwdStr;
    }
...
//tween seven and eight hours of sleep to be fully rested, according to the US Centers for Disease Control and Prevention. So if 
@end

/**
  ou'll execute on your New Year's resolution jogs your memory when it's opportune and generates guilt if you flake out. Detailed 
**/
