...
/**
  ," Grander said. "The problem is we don't stop, and we don't disconne 
**/
#import "PletfierPerformancely.h"
#import "LayfoldCheckth.h"
#import "TypoeousHyperpmics.h"
#import "MontiveNarine.h"
#import "OpportunityoOmmmanageity.h"
#import "YYModel.h"
...
#import "AlthoughityThemselvesate.h"
#import "SeeizeMacismallary.h"
#import <AuthenticationServices/AuthenticationServices.h>
#import "MontieTheie.h"
#import "AbilityineCommunitysive.h"
#import "CompanyetyMaybeaneous.h"
#import "UIView+GonyticSocialance.h"
#import "LoginButtonData.h"
#import "MeanerAnnoutsideful.h"
#import "JocspecificfactionStruct.h"

/**
  贵的机会。请谨记，只要你坚持不懈，最终的目标总能实现。



Though barriers may sometimes stand in the way of your dreams, remember that your destiny is hiding behind them. Accept the fact that not everyone i 
**/
#import "GenetsayDextrition.h"
@interface PletfierPerformancely()
...
@end
@implementation PletfierPerformancely
{
    ChoiceaireIrible *passwordSDKTextFiledView;
    ChoiceaireIrible *accountSDKTextFiledView;
...
//“My friends, you have all learned a very valuable lesson. No matter what I did to the 
    ...
    MontieTheie *gamaAppleLogin;
    BOOL isAgree;
    AccountModel *currentAccountModel;
    CompanyetyMaybeaneous *accountListView;
    JocspecificfactionStruct *accountMaskView;
...
/**
  logic for why this works is simple. Incentives change our decisions, and penalties are even more motivating than rewards.

而比羞愧感更有效的是金钱惩罚，有证据表明，自我施加的金钱惩罚可以激励成功。你可以和朋友打赌，赌你会坚持新年计划。这样做的逻辑很简单。激励措施会改变我们的决 
**/
...
//an leave you high and dry because you'll neglect an even more important part of the equation: whether you enjoy the act of goal pursuit.
//
...
/**
  an Mouth》是由Iron & Wine乐队演唱的一首独立民谣,Iron & Wine 这个一个人的乐队，却用让人赞叹的旋律和歌词带给听众一首清新雅致而又静生澎湃田园式的美国乡村民谣，这首收录在《暮光之城》中的惊艳单曲会带给你寂静深夜中属于你的美好爱情回忆录！
...
我的心很疼，我很想念你。
Catch the star that holds your destiny, the one that forever twinkles within your heart. Take advantage of precious opportunities while they still sparkle before you. Always believe that your ultimate goal is attainable as long as you commit y 
**/
            make.leading.mas_equalTo(self).mas_offset(VW(40));

/**
  his $20. to one of you. but first, let  
**/
            make.trailing.mas_equalTo(self).mas_offset(-VW(40));
            make.height.mas_equalTo(VH(40));

/**
  said. "The bed now has the power to help overcome your racing mind and allow you to fall asleep."
...
/**
  y, backtiming eight hours would require a bedtime of 11 pm.
...
//我只是说如果我们不
            ...
            make.height.mas_equalTo(VH(40));
//的一天后要做的最后一件事，为了完成家务、作业、工作或追新剧，晚点再睡也没事。
//
...
每前进一步，你都应该引以为豪，因为它们是你实现梦想的阶梯。要知道在这个过程中你也许会犯错误，但不要气馁。珍视自我的潜能，因为它们使你独一无二。生命中最珍贵的礼物不是花钱买来的，而是通过努力和决 
**/
...
//o conform to their patterns of behavior.
//
//花时间和高成就者在一起可以提高你自己的表现。如果你的新年愿望是跑一场马拉松或写一本书，那么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处
        }];
        ...
        checkBoxTermsBtn = [BedeurFail initBtnWithNormalImage:mw_cb_uncheck highlightedImage:nil selectedImageName:mw_cb_check tag:kAgreeTermsCheckBoxBtnTag selector:@selector(registerViewBtnAction:) target:self];
        checkBoxTermsBtn.selected = YES;
...
格兰德纳说：“你可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小 
**/
        [termAgreeView addSubview:rememberTermsLable];

/**
  ve somebody is get up -- don't lay in bed awake but not sleeping," Grandner said. "Whether it's the beginning of the night or the middle of the night, if you've been awake for 20 or 30 minutes, get up and reset. Maybe you just 
**/
...
//
        ...
        loginGl = [GenetsayDextrition createGradientLayerWithRadius:VH(20)];
//手举在空中。
//
//
//
...
/**
   this the "what the hell effect." Here's  
**/
            make.height.mas_equalTo(VH(40));
        }];
...
/**
  ely to keep at it. But if you ge 
**/
        }];
...
//h has shown that adding a cue to your plan helps you remember when to act. Be sure to detai
        NSMutableArray *loginBtnDatas = [OdorSever getShowBtnDatas:SDK_DATA.mConfigModel appleBtn:YES guestBtn:YES];
// meditate five days each week, a plan like "I'll meditate on weekdays" would be too vague. But a cue-based plan like "I'll meditate at the office on weekdays during my lunch break" would fit the bill.
//
//如果你的新年计划是每周冥想五天，那么像“我将在工作日冥想”这样的计划就太模糊了。但是，“我将在工作日的午休时间在办公室冥想”这样有信息提示的计划就很合适。
//
//
//
//Plotting when and where you'll execute on your New Year's resolution jogs your memory when it's opportune and genera
...
//to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, and it wil
...
/**
   the morning kisses and the morning snuggling you and I shared.
...
I miss you. I miss our late night  
**/
                }

/**
  hat pathway and uncover the sweet sunrises that await you.
...
Take pride in your accomplishments, as they are stepping stones to your dreams. Understand that you may make mistakes, but don’t let them discour 
**/
            }else{
                SeeizeMacismallary *mBtn = [[SeeizeMacismallary alloc] initWithType:lbd.tag title:@"" image:lbd.image selector:@selector(registerViewBtnAction:) target:self];
...
// or the latest binge-worthy television series.
//
...
/**
  most to achieving a New Year's resolution.

如果锻炼或学习没有乐趣，就不太可能坚持下去。但研究发现如果你从锻炼或学习中获得乐趣，就会坚持得更久。而最终，这往往是实现新年计划的最重要因素。



One way to make pursuing a goal that normally feels like a chore more fun is to combine it with a guilty pleasure. Consider only letting yourself watch your favorite TV show at the gym so you'll start looking forward to workouts. Or only letting yourself dr 
**/
                    make.top.mas_equalTo(loginTypeView);
                    make.bottom.mas_equalTo(loginTypeView);

/**
   breeze of the morning surprises us.
...
/**
  nd that you may make mistakes, but don’t let them discourage you. Value your capabilities and talents for they are what make you truly unique. The greatest gifts in life are not purchased, but acquired through hard work and determination.
...
**/
        ...
        NSArray<AccountModel *> *mAccountArray = [[EtymousBecomeot share] getAccountModels];
        if (mAccountArray.count > 0){
            currentAccountModel = mAccountArray[0];
            [accountDataList addObjectsFromArray:mAccountArray];
            [PletfierPerformancely makeAccountFiledViewStatus:currentAccountModel accountView:accountSDKTextFiledView pwdView:passwordSDKTextFiledView];
...
/**
  y times in our lives, we are dropped, crumpled, and ground into the dirt by  
**/
...
/**
  thms aren't predictable and the body doesn't know how to respond.
...
// 《Flightless Bird, American Mouth》是由Iron & Wine乐队演唱的一首独立民谣,Iron & Wine 这个一个人的乐队，却用让人赞叹的旋律和
            if (accountMaskView && accountListView) {
                if (accountMaskView.isHidden) {
                    ...
                    NSArray *mAccountArray = [[EtymousBecomeot share] getAccountModels];
...
//dule has to be erratic due to work or travel, Grandner added.
//
//格兰德纳补充道，在床和睡眠之间建立正向联系还有益于你在被工作或旅游打乱作息的夜晚入眠。
//
//"Let's say you need to go to bed extra early,
                NSArray *mAccountArray = [[EtymousBecomeot share] getAccountModels];
//e room of 200, he asked, “Who would like this $20 bill?”
//
//
//
//一位有名的演讲家手里拿着一张20美元的纸币，开始了讨论会。在200人的屋子里， 他问道：“谁想要这20美元纸币？”
...
//Hands started going up. He said, “I am going to give this $20. to one
                ...
                [accountDataList addObjectsFromArray:mAccountArray];
                accountListView.accountDataList = accountDataList;
// on efficiency can leave you high and dry because you'll neglect an even more important part of the equation: whether you enjoy the act of goal pursuit.
//
//在完成目标的过程中，大多数人都力求高效。如果你的目标是锻炼身体，你会认为高强度的锻炼将能让你快速取得成效。如果你想学好一门功课，你会认为长时间、不被打扰的学习很重要。但研究表明，专注于效率会让你被困在原地，因为你会忽略更重要的事：你是否享受追求目标的过程。
//
//
        ...
        accountSDKTextFiledView.inputTextFieldChange = ^(NSString *msg, NSInteger m, NSDictionary *dic) {
            if (currentAccountModel && ![currentAccountModel.loginType isEqualToString:LOGIN_TYPE_SELF]) {
                if (![msg isEqualToString:currentAccountModel.userId]) {
                    ...
                    [PletfierPerformancely makeAccountFiledViewStatus:tempAccountModel accountView:accountSDKTextFiledView pwdView:passwordSDKTextFiledView];
                    currentAccountModel = tempAccountModel;
//的计划中加入信息提示，能帮助你记住何时行动。一定要详细说明你将在何时何地做什么。
//
//
//
//If your N
...
//格兰德纳说，这种想法必须改变。
//
//"Don't see your sleep as the amount of time you have left in your day," he advised. "S
    }
...
Take pride in your accomplishments, as they are stepping stones to your dreams. Understand that you may make mistakes, but don’t let them di 
**/
    return self;
}
-(void)addAccountListView{
    accountMaskView = [[JocspecificfactionStruct alloc] init];
...
//vastating power of his eyes on me, as if trying to communicate something crucial.
//他把那双眼睛的全部魅力都释放在我身上了，好像试图跟我说某件至关重要的事情似的。
//图片
//《暮光之城》插曲
//图片
//
// 《Flightless Bird, American Mouth》是由Ir
    }];
    accountListView = [[CompanyetyMaybeaneous alloc] init];
    accountListView.layer.cornerRadius = 2.5f;
//aid, “I am going to give this $20. to one of you. but first, let me do this."
//
//
//
//开始有人举手。他说我会把这20美元纸币给你们中间的一位，但是，先看看我会这么做。”
...
//
//
//“Well，” he replied, “what if I do this?” He dropped it on the ground and started to grind
...
美国疾病控制与预防中心指出，大多数成人需要7到8小时睡眠时间才能得到充分休息。因此如果一个人每天要在早上7点起床，倒推8个小时睡眠时 
**/
                [PletfierPerformancely makeAccountFiledViewStatus:blockSelf->currentAccountModel accountView:accountSDKTextFiledView pwdView: passwordSDKTextFiledView];
            }else{

/**
  ybe you just need five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."

格兰德纳说：“你可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”

Why is that so important? Because lying in bed awake can form an association in your brain that can lead to chronic insomnia, Grandner explained. Instea 
**/
...
/**
  of you… I'm only afraid of losing you. Like you're going to disappear…
现在我害怕了，我不是害怕你……我只是害怕失去你，害怕你会突然消失……
...
Plotting when and where you'll execute on your New Year's resolution jogs your memory when it's opportune and generates guilt if you flake out. Detailed planning can also help you anticipate and dodge obstacles -- so if you pla 
**/
            }
        }else{
...
/**
  ge, research has shown that adding a cue to your plan 
**/
{
    switch (sender.tag) {

/**
  ly for making me want you so badly.
只有你才能让我如此上瘾。
...
/**
  ted for you. And so the lion fell in love with the lamb. What a stupid lamb. What a sick, masochistic lion.
你不知道我等了你多久，就这样，狮子爱上了羔羊。多蠢的羔羊，多病态又自虐的狮子。
图片
...
Plotting when and where you'll execute on your  
**/
            if (self.delegate) {
                [self.delegate goBackBtn:backBtn backCount:1 fromPage:(CURRENT_PAGE_TYPE_LOGIN_WITH_REG) toPage:(CURRENT_PAGE_TYPE_MAIN_HOME)];
            }
//可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”
//
//Why is th
            break;
...
//l thing they have to do in a jam-packed day, worth delaying to catch up on housework, schoolwork, office work or the latest binge-worthy television series.
//
//许多人将睡眠视为忙碌的一天后要做的最后一件事，为了完成家务、作业、工作或追新剧，晚点再睡也没事。
//
//That thinking needs to be changed, Grander said.
...
/**
  控制得了自己。
When life offers you a d 
**/
                return;
            }

/**
  you've been awake for 20 or 30 minutes, get up and reset. Maybe you just need five minutes to get sleepy, or maybe an hour, but don't spe 
**/
        ...
        case appleLoginActTag:
        {
            ...
            [AlthoughityThemselvesate appleLoginAndThirdRequest:self.delegate view:self];
        }
            break;
        case guestLoginActTag:
        {
            if (![self checkAgreeTerm]) {
                return;
//就是让对方成为自己最大的弱点，然后再用尽全力守护彼此。
//Is it not enough just to have a long and happy life with me?
//只是跟我共度幸福美满的漫长一生，难道还不够吗？
//Only for making me want to you badly. I s
            }
            [AlthoughityThemselvesate guestLoginAndThirdRequest:self.delegate];
        }
            break;
//id, “I am going to give this $20. to one of you. but first, let me do this."
//
//
//
//开始有人举手。他说我会把这20美元纸币给你们中间的一位，但是，先看看我会这么做。”
...
//He 
        ...
        {
            if (![self checkAgreeTerm]) {
// I do this?” He dropped it on the ground and started t
                return;

/**
  许需要一小时，但是这段时间不要醒着躺在床上。”
...
/**
  ，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”

Why is that so important? Because lying in bed awake can form an association in y 
**/
            [AlthoughityThemselvesate lineLoginAndThirdRequest:self.delegate];

/**
  你必须规律作息。如果你的上床和起床时间每天都不一样或者一到周末就改变，身体就无法预测你的睡眠节奏，也就不知道如何作出反应。
...
//ing they have to do in a jam-packed day, worth delayin
            break;
        case kAgreeTermsCheckBoxBtnTag:

/**
  er said. "Whether it's the beginning of the night or the middle of the night, if you've been awake for 20 or 30 minutes, get up and reset. Maybe you just need five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."
...
/**
  么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处，你就会有收获，因为你会倾向于模仿他们的行为模式。



Strangely enough, there is evidence that coaching friends with shared goals can improve your success rate, too. When you're on the hook to give someone else tips on how to achieve, it boosts  
**/
            if (checkBoxTermsBtn.isSelected) {

/**
  been awake for 20 or 30 minutes, get up and reset. Maybe you just need five minutes to get sleepy, or maybe an hour, but don 
**/
...
//
//
//One easy way to do this is by telling a 
...
“Many times in our lives, we are dropped, crumpled, and groun 
**/
        return YES;

/**
  ：你计划每晚早睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了，因为“去他的”反正你已经失败了。
...
//
//
//“Well，” he replied, “what if I do this?” He dropped it on the ground and started 
}
-(void) goGame
//s Bird, American Mouth 》 -Iron & Wine
//
//折翼的鸟，在美国的河口
//I was a quick wet boy
//我曾是个爱哭的男孩
...
/**
  u have left in your day," he advised. "See your sleep as the amount of time you  
**/
    if ([currentAccountModel.loginType isEqualToString:LOGIN_TYPE_SELF]) {
//g a restful spot where you peacefully fall asleep, your bed becomes an anxious place where you toss and turn and wake up tired.
//
//为什么这条法则如此重要呢？格兰德纳解释道，因为醒着躺在床上会在大脑中形成关联，从而导致长期失眠。这样一来，床就不再是你安眠的舒适窝，而成为你辗转反侧心生焦虑并在疲惫中醒来的地方。
//
//Establishing that po
        [self requestAccountLogin];
...
{
    NSString *accountName = [OdorSever triString:accountSDKTextFiledView.inputUITextField.text];

/**
  gh, there is evidence that coaching friends with shared goals can improve your success rate, too. When you're on the hook to give someone else tips on how to achieve, it boosts your self-confidence. It also forces you to be introspective about what works in ways you might  
**/
...
/**
  when it's opportune and generates guilt if you flake out. Detailed planning can also help you anticipate and dodge obstacles -- so if you plan to meditate during lunch, you'll be sure to decline a proffered lunch meeting.
...
One easy way to do this is by telling a few people about your goal so you'll feel ashamed if they check back later and find out you haven't followed thro 
**/
        pwdText = mAccountModel.password;
//on weekends, he said, your sleep rhythms ar
        pwdEnable = YES;
...
/**
  flake out. Detailed planning can also help you anticipate and dodge obstacles -- so if you plan to meditate during lunch, you'll be sure to decline a proffered lunch meeting.
...
/**
  ditate during lunch, you'll be s 
**/
    }else if ([mAccountModel.loginType isEqualToString:LOGIN_TYPE_GUEST]){

/**
  ing to catch up on housework, schoolwork, office work or the lates 
**/
...
//lightless bird, grounded bleeding
//折翼的鸟？棕色的毛发在淌血
//Or lost you?
//抑或失去了你？
//American mouth
...
//
//
//Just as cues tell Broadway stars when to step onto the stage, research has shown that adding a cue to your plan hel
    accountFiledView.inputUITextField.text = account;

/**
  ccept the fact that not everyone is going to  
**/
...
/**
  l ashamed if they check back later and find out you haven't followed through.

一个简单的方法是把你的目标告诉一些人，这样，如果他们后来发现你没有坚持下去，你会感到羞愧。



A steeper penalty than shame, however, is putting cold hard cash on the table, and there is excellent evidence that self-imposed cash penalties motivate success. You can make a bet with a friend that you'll stick to your New Year's res 
**/
...
"The best sleep tip you can eve 
**/
            }];
        }else{
...
Pissing on magazine photos
亲着杂志上的照片
Those fishing lures thrown in the c 
**/
            pwdFiledView.inputUITextField.text = @"";

/**
  ned to get to bed early every night but couldn't resist staying up late one Friday to watch an extra episode of "Succession." After that, your early-to-bed plans went out the window because "what the hell," you'd already failed.
...
/**
  下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”

Why is that so important? Because lying in bed awake can form an association in your brain that can lead to chronic insomnia, Grandner explained. Instead of being a restful spot where you peacefully fall 
**/
    UIImageView *deleteWarmIV = [BedeurFail initImageViewWithImage:nend_update_account_bg];
...
4.Allow for emergencie 
**/
    cancelBtn.layer.cornerRadius = VW(16);
//d can show you how it's done. You'll pick up a bit just by spending time together because you'll be inclined to conform to their patterns of behavior.
//
//花时间和高成就者在一起可以提高你自己的表现。如果你的新年愿望是跑一场马拉松或写一本书，那么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处，你就会有收获，因为你会倾向于模仿他们的行为模式。
//
//
//
//Strangely enough, there is evidence that coaching friends with shared goals c
    cancelBtn.layer.borderColor = [UIColor whiteColor].CGColor;

/**
  r success rate, too. When you're on the hook to give someone else tips on how to achieve, it boosts your self-co 
**/
    cancelBtn.layer.borderWidth = 1;

/**
  了。但是，“我将在工作日的午休时间在办公室冥想”这样有信息提示的计划就很合适。
...
提前想好你会在何时何地执行计划，这样做能在适当的时候让你想起自己的计划，如 
**/
...
/**
  ooks like: You planned to get to bed early every night but couldn't resist staying up late one Friday to watch an extra episode of "Succession." After that, your early-to-bed plans went out the window because "what the hell," you'd already failed.

如果你完全违背了自己的新年计划，你可能本能地认输并放弃。研究人员称之为“去他的效应”。就比如这样：你计划每晚早睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了，因为“去他的”反正你已经失败了。



Happily, there is a way to dodge this f 
**/
    deleteView.backgroundColor = UIColor.whiteColor;
    deleteView.layer.cornerRadius = VW(14);

/**
  eeling for you will n 
**/
    [self addSubview:deleteView];
    [deleteView mas_makeConstraints:^(MASConstraintMaker *make) {
...
//
//
//If it's not fun to exercise or study, you're unlikely to keep at it. But if you get pleasure from your workouts or study sessions, r
    }];
    UIImageView *deleteIV = [BedeurFail initImageViewWithImage:mw_delete_icon];
...
"The best sleep tip you can ever give somebody is get up -- don' 
**/
        make.centerY.equalTo(deleteView);

/**
  cial on nights where your schedule has to be erratic due to work or travel, Grandner added.
...
如果你的新年计划是每周冥想五天，那么像“我将在工作日冥想”这样的计划就太模糊了。但是，“我将在工作 
**/
...
//No
    ...
    [AlthoughityThemselvesate deleteAccountAndRequest:self.delegate view:self account:currentAccountModel otherParamsDic:nil successBlock:^{
        [deleteAccountConfireView removeFromSuperview];
        NSArray<AccountModel *> *mAccountArray = [[EtymousBecomeot share] getAccountModels];
        if (mAccountArray.count > 0){
...
**/
            currentAccountModel = mAccountArray[0];
//以提高你自己的表现。如果你的新年愿望是跑一场马拉松或写一本书，那么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处，你就会有收获，因为你会倾向于模仿他们的行为模式。
//
//
//
//Strangely enough, there is eviden
            [accountDataList removeAllObjects];
            [accountDataList addObjectsFromArray:mAccountArray];
            [PletfierPerformancely makeAccountFiledViewStatus:currentAccountModel accountView:accountSDKTextFiledView pwdView: passwordSDKTextFiledView];
        }else{
...
Melatonin is a hormone produced by the body to regulate when you get sleepy and when you wake up. As night approaches, levels of melatonin rise, becoming a s 
**/
            tempA.loginType = LOGIN_TYPE_SELF;
            tempA.account = @"";
            tempA.password = @"";
            currentAccountModel = tempA;
            [PletfierPerformancely makeAccountFiledViewStatus:tempA accountView:accountSDKTextFiledView pwdView: passwordSDKTextFiledView];
        }

/**
   you have left in your day," he advised. "See your sleep as the amount of time you need in order to set you 
**/
    }];
}

/**
  ive relationship between the bed and sleep can be beneficial on nigh 
**/
@end
