...
/**
  ike a 10 pm bedtime every night) but giving yourself one or two get-out-of-jail-free cards each week, you can get better results than by setting either 
**/
#import "SeasonficVill.h"
#import "CorticatorAffectward.h"
#import "NonagenitorUntil.h"
//果你完全违背了自己的新年计划，你可能本能地认输并放弃。研究人员称之为“去他的效应”。就比如这样：你计划每晚早睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了，因为“去他的”反正你已经失败了。
//
//
//
//Happily, there is a way to dodge this fate. By setting tough goals (like a 10 pm bedtime every night) but giving
#import "MergessRelateitious.h"
#import "TheableZyg.h"
#import "YYModel.h"

/**
  这听起来只是想法上的一个小小改变，但却很重要。
...
//f melatonin rise, becoming a signal to the body that it's time for bed. Production of melatonin is stopped by light -- so levels naturally fall as daylight approaches, getting you ready to greet the day.
//
...
#import "FlectiblePantator.h"

/**
  iends who've made it to the finish line (literally or figuratively) and can show you how it's done. Yo 
**/
#import "SarcooryPoiequite.h"
#import "MusuleSphen.h"
//re to detail when and where you'll follow through.
//
//正如出场提示告诉百老汇明星何时登台一样，研究表明，在你的计划中加入信息提示，能帮助你记住何时行动。一定要详细说明你将在何时何地做什么。
//
//
//
//If your New Year's resolution is to meditate five days each week, a plan like "I'll meditate on weekdays" would be too vague. But a cue-based plan 
#import "EquiaciousAcuacious.h"
#import "InsideAuthor.h"
#import "AccountModel.h"
#import <FBSDKCoreKit/FBSDKCoreKit.h>
@interface ArtistBegin()
@property (nonatomic,strong)UIView *sdkContentView;

/**
   I have woken up right beside you.
...
// an hour, but don't spend that time awake in bed."
//
...
@end
@implementation ArtistBegin{
    SDKPage sdkPageType;

/**
  u. Value your capabilities and talents for they are what make you truly unique. The greatest gifts in life are not purchased, but acquired through hard work and determination.



每前进一步，你都应该引以为豪，因为它们是你实现梦想的阶梯。要知道在这个过程中你也许会犯错误，但不要气馁。珍视自我的潜能，因 
**/
    BromwardVeraciitious *mLoginWithRegView;
    BromwardVeraciitious *mMainHomeView;
//t to have a long and happy life with me?
...
//When life offers you a dream so far beyond any of y
    BromwardVeraciitious *mWelcomeBackView;
}
...
褪黑素是身体分泌的一种激素，你什么时候感到困，什么 
**/
{
    self = [super init];

/**
  body is get up -- don't lay in bed awake but not sleeping," Grandner said. "Whether it's the beginning of the night or the middle of 
**/
    if (self) {
...
Though barriers may sometimes stand in the way of your dreams, remember that your destiny is hiding behind them. Accept the fact that not everyone is going to approve of the choices you’ve made, have faith in your judg 
**/
        }

/**
  避免阻碍——如果你计划在午餐时冥想，你一定会拒绝别人提出的午 
**/
            break;
...
/**
  thway and uncover the sweet sunrises that await you.
...
Take pride  
**/
    [VesicallyGreatless logWithEventName:AD_EVENT_OPEN_LOGIN_SCREEN parameters:nil type:AdType_Appflyer|AdType_Firebase];
}
...
//
//
//Strangely enough, there is evidence that coaching friends with shared goals can improve your success rate, too. When you're on the hook to give someone else tips on how to achieve, it boosts your self-confidence. I
{
    NSArray<AccountModel *> *accountModels = [[EtymousBecomeot share] getAccountModels];
...
/**
  akness ,then going all you can do to protect each other.
爱就是让对方成为自己最大的弱点，然后再用尽全力守护彼此。
Is it not enough just to have a long and happy life  
**/
            [self addWelcomeView];
        }
...
/**
  ances that come our way. We feel as though we are worthless；but no matter what happened or what will happen 
**/
...
/**
  ur destiny is hiding behind them. Accept the fact that  
**/
...
#pragma mark -頁面添加部分
//ng of the night or the middle of the night, if you've been awake for 20 or 30 minutes, get up and reset. Maybe you just need five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."
//
//格兰德纳说：“你可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”
//
//Why is that so important? Because lying in bed awa
-(BromwardVeraciitious *)addLoginWithRegView
//Many times in our lives, we are dropped, crumpled, and ground into the dirt by the decisions we make and the circumstances that come our way. We feel as though we are worthless；but no matter what happened or what will happen, you will never lose your value."
//
//
//
...
{
    if (is_Version2) {
//地上，开始用皮鞋使劲踩踏。然后他拣起又脏又皱的纸币，“现在，还有人要它吗？”
//
//
//
//Still the hands went into the air.
...
//
//空
        mLoginWithRegView = [[EquiaciousAcuacious alloc] initView];
//你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许
    }else{
        mLoginWithRegView = [[PheniceSexagenmyselfaire alloc] initView];
    }
...
// of the morning surprises us.
//
//我想念阳光照在你的脸上，早晨的寒风让我们感到惊讶。
//
//I miss the morning kisses and t
...
Most adults need between seven and eight hours of sleep to be fully rested, accor 
**/
    }

/**
  vercome your racing mind and allow you to fall asleep."
...
**/
    mWelcomeBackView = [[FodindustryismCome alloc] initView];
...
Just as cues tell Broadway stars when to step onto the stage, research  
**/
-(BromwardVeraciitious *)addTermsView
{
    AbilityineCommunitysive *mTermsView = [[AbilityineCommunitysive alloc] initWithCompleter:^{
    }];
...
If it's not fun to exercise or study, you're unlikely to keep at it. But if you get pleasure from your workout 
**/
-(BromwardVeraciitious *)addBindAccountView
{
//ions, and penalties are even more motivating than rewards.
//
//而比羞愧感更有效的是金钱惩罚，有证据表明，自我施加的金钱惩罚可以激励成功。你可以和朋友打赌，赌你会坚持新年计划。这样做的逻辑很简单。激励措施会改变我们的决定，而惩罚甚至比奖励让我们更有动力。
//
//3.Make i
    SarcooryPoiequite *view = [[SarcooryPoiequite alloc] initView];
    [self addSubSdkLoginView:view];
    return view;
}
//打赌，赌你会坚持新年计划。这样做的逻辑很简单。激励措施会改变我们的决定，而惩罚甚至比奖励让我们更有动力。
//
//3.Make it fun
//
//找些乐趣
...
-(BromwardVeraciitious *)addChangePasswordView
{
...
//
//
    NonagenitorUntil *view = [[NonagenitorUntil alloc] initView];
// a chore more fun is to combine it with a guilty pleasure. Consider only letting yourself watch your favorite TV show at the gym so you'll start looking forward to workouts. Or only letting yourself drink a mocha latte during study sessions so there is a hook to get you to the library.
//
//想要把追求一个辛苦的目标变得更有趣，方法就是把它与有罪恶
    [self addSubSdkLoginView:view];
    return view;
}
//not everyone is going to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, and it will lead you to your destiny’s path. Follow that pathway and uncover the sweet sunrises that await you.
//
//
//
...
/**
  d five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."

格兰德纳说：“你可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”

Why is that so important? 
**/
    MergessRelateitious *view = [[MergessRelateitious alloc] initView];
    [self addSubSdkLoginView:view];
...
Though barriers may sometimes stand in the way of your dreams, remember that your destiny is hiding behind them. Accept the fact that not everyone is going to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, and it will lead you to your destiny’s path. Follow that pat 
**/
-(void)addFindPasswordView

/**
  office work or the latest binge-worthy television series.
...
/**
  nd friends who've made it to the finish line (literally or figuratively) and can show you how it's done. You'll pick up a bit just by spending time together because you'll be inclined to conform to their patterns of behavior.

花时间和高成就者在一起可以提高你自己的表现。如果你的新年愿望是跑一场马拉松或写一本书，那么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处，你就会有收获，因为你会倾向于模 
**/
        InsideAuthor *view = [[InsideAuthor alloc] initView];
        [self addSubSdkLoginView:view];
...
As long as i live, my feeling for you will never fade.
我对你的爱，至死不渝。
Now I'm afraid. I'm not afraid of you… I'm only afraid of losing you. L 
**/
    }else{
        TheableZyg *view = [[TheableZyg alloc] initView];
        [self addSubSdkLoginView:view];
    }
//准备好迎接新的一天。
//
//To work properly, the release of this hormone needs to occur at regular times, said cli
}
-(void)addHomeView
{
    for (UIView *subView in [self sdkContentView].subviews) {
    ...
    if (is_Version2) {
        mMainHomeView = [[MusuleSphen alloc] initView];
    }else{
        mMainHomeView = [[FlectiblePantator alloc] initView];

/**
  呼吸。
I'd never given much thought to how I wo 
**/
    }
...
临床心理学家、睡眠专家迈克尔·格兰德纳指出，为了让身体正常分泌褪黑 
**/
}
-(void)addSubSdkLoginView:(BromwardVeraciitious *)mSDKBaseView
{
...
“Well，” he replied, “what if I do this?” He dropped it on the ground and started to grind it 
**/
        subView.hidden = YES;
...
//
//
//“Many times in our lives, we are dropped, crumpled, and ground into the dirt by the decisions we make and the circumstances that come our way. We feel as though we are worthless；but no matter what happened or wh
    }

/**
  ve the strength to stay away from you anymore.
我再也没有离开你的力气了。
...
/**
  ss you and I miss us being together physically.
...
/**
   your brightest dreams come true. Give your hopes everything you’ve got and you will catch the star that holds your destiny.
...
I miss the way the sun shines at your face and the way the cold b 
**/
...
{
//e hook to give someone else tips on how to achieve, it boosts your self-conf
    BromwardVeraciitious *mView;
    switch (toPage) {
        case CURRENT_PAGE_TYPE_AUTO:
            break;
//nd and allow you to fall asleep."
//
//他说：“比如说你需要提早上床睡觉。床将能帮助你安抚纷繁的思绪，让你酣然入睡。”
//
//3. Change your attitude about sleep
...
//改变对睡眠的心态
//
//Many people view sleeping as the final thing they have to do in a jam-packed day, worth delaying to catch up on housework, schoolwork, office work or the latest binge-worthy tel
        case CURRENT_PAGE_TYPE_FIND_PWD:

/**
  hat twinkles in your heart for you alone are capable of making your brightest dreams come true. Give your hopes everything you’ve got and you will catch the star that holds your destiny.
...
/**
  star that twinkles in your heart, and it 
**/
            [self addHomeView];
...
"Now you know when you have to stop and get ready to go to bed whether you're done or not," Grander said. "The problem is we don't stop 
**/
            break;
...
/**
  fell in love with the lamb. What a stupid lamb. What a sick, masochistic lion.
...
**/
        case CURRENT_PAGE_TYPE_BIND_ACCOUNT:
            mView = [self addBindAccountView];
            break;
        case CURRENT_PAGE_TYPE_TEARMS:
        {
            mView = [self addTermsView];

/**
  睡，但忍不住在某个星期五熬夜多看一集《继承之战》，此后你的早睡计划泡汤了，因为“去他的”反正你已经失败了。
...
**/
            ...
            break;
//建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不着，就起床调整一下。也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”
//
//Why is that so important? Because lying in bed awake can form an association in your brain that can lead to chronic insomnia, Grandner explained. Instead of being a restful spot where you p
        default:
            break;
    }
...
/**
  dence that self-imposed cash penalties motivate success. You can make a bet with a friend that yo 
**/
...
/**
  rogress. If you want to ace a class, you assume long, distraction-free study sessions are key. But research has shown that focusing on efficiency can leave you high and dry 
**/
            break;
// time you have left in your day," he advised. "See your sleep as 
        case CURRENT_PAGE_TYPE_MAIN_HOME:
            if (mMainHomeView) {

/**
  ave faith in your judgment, catch the star that twinkles in your heart, and it will lead you to your 
**/
                mMainHomeView.hidden = NO;
            }else{
                [self addHomeView];
            }
//who've made it to the finish line (literally or figuratively) and can show you how it's done. You'll pic
            break;
...
//格兰德纳补充道，或许这听起来只是想法上的一个小小改变，但却很重要。
//
//Most adults need between seven and eight hours of sleep to be fully rested, according to the US Centers for Disease Contr
            }else{
                [self addWelcomeView];
//害怕了，我不是害怕你……我只是害怕失去你，害怕你会突然消失……
...
/**
  巴的或者被折磨，对周围爱你的人来说你仍然是无可替代的。我们生活的价值不在于我们做了什么，或者我们认识谁，生活的价值在于我们是谁。”
...
/**
  kdays during my lunch break" would fit the bill.
...
我喜欢夜晚。没有黑暗，我们永远都看不见星 
**/
    AccountModel *rData = loginResopnse.data;
    rData.loginType = thirdPlate;
    [[EtymousBecomeot share] saveAccountModel:rData];
    [[EtymousBecomeot share] saveLoginType:thirdPlate];
    if ([LOGIN_TYPE_SELF isEqualToString:thirdPlate]) {
    }
    if (loginResopnse.code == BJResponseCode_OK_Register) {
        [VesicallyGreatless logWithEventName:AD_EVENT_REGISTER_SUCCESS parameters:nil type:AdType_Appflyer|AdType_Firebase];
//s closed the fair
//当警
        [VesicallyGreatless logWithEventName:FBSDKAppEventNameCompletedRegistration parameters:nil type:AdType_FB];
        [VesicallyGreatless logWithEventName:AD_EVENT_COMPLETE_REGISTRATION_IOS parameters:nil type:AdType_FB];
...
提前想好你会在何时何地执行计划，这样做能在适当的时候让你想起自己 
**/
}
...
//一本书，那么明智的做法是开始与那些已经实现这一目标、可以告诉你如何做到的朋友们在一起。仅仅是和他们相处，你就会有收获，因为你会倾向于模仿他们的行为模式。
//
//
//
...
/**
  how at the gym so you'll start looking forward to workouts. Or only letting yourself drink a mocha latte during study sessions so there is a hook to get you to the library.
...
// own performance. If your New Year's resolution is to run a marathon or write a book, you'd be wise to start hanging around friends
    }
}
...
Only for making me want you so badly.
只有你才能让我如此上瘾。
I don't have the strength to stay awa 
**/
        [weakSelf.sdkContentView mas_updateConstraints:^(MASConstraintMaker *make) {

/**
  uired through hard work and determination.
...
**/
            make.centerY.equalTo(@(0));
//ho still wants it?” Still the hands were up in the air.
//
//
//
//他开始把这张纸币揉皱，然后他问道:“还有人想要它吗？”仍然有很多手举在空中。
...
//格兰德纳补充道，或许这听起来只是想法上的一个小小改变，但却很重要。
//
//Most adults need between seven and eight hours of sleep to be fully rested, according to the US Centers for Disease Control and Prevention. So if a person needed to rise at 7 am each day, backtim
...
/**
  也许你只需要5分钟就能感觉到困意，也许需要一小时，但是这段时间不要醒着躺在床上。”
...
/**
  么样呢？”他把纸币扔到地上，开始用皮鞋使劲踩踏。然后他拣起又脏又皱的纸币，“现在，还有人要它吗？”
...
/**
  炼或学习没有乐趣，就不太可能坚持下去。但研究发现如果你从锻炼或学习中获得乐趣，就会坚持得更久。而最终，这往往是实现新年计划的最重要因素。



One way to make pursuing a goal that normally feels like a chore more fun is to combine it with a guilty pleasure. Consider only letting yourself watch your favorite TV show at th 
**/
...
//
//There are not enough ways to express
