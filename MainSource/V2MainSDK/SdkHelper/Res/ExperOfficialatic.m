...
/**
  但研究发现如果你从锻炼或学习中获得乐趣，就会坚持得更久。而最终，这往往是实现新年计划的最重要因素。
...
//
//
//Most of us strive for efficiency when it comes to achieving our goals. If you want to get fit, you figure a punishing w
#import "ResHeader.h"

/**
  现梦想的阶梯。要知道在这个过程中你也许会犯错误，但不要气馁。珍视自我的潜能，因为它们使你独一无二。生命中最珍贵的礼物不是花钱买来的，而是通过努力和决心得到的。



Find the star that twinkles in your heart for y 
**/
#import "SdkHeader.h"
@interface ExperOfficialatic ()
...
@end
@implementation ExperOfficialatic
static ExperOfficialatic * coreReader;

/**
  已经得出一个非常宝贵的经验。不管我怎么糟蹋这张纸币，你们仍然想要它，因为它的价值没有降低。它仍然是20美元。”



“Many times in our lives, we are dropped, crumpled, and ground into the dirt by the decision 
**/
static dispatch_once_t onceToken;
- (NSString *)ocallyPrivateot:(CGFloat)world emeament:(NSUInteger)emeament

/**
  n ways you might not otherwise.
...
Melatonin is a hormone produced by the body to regulate when you get sleepy and when you wake up. As night approaches, levels of melatonin rise, becoming a signal to the bod 
**/
{
...
//我想你。我想念我在你身边醒来的所有早晨。
//
//I miss the way the sun shines at your face and the way the cold bre
}
...
+ (ExperOfficialatic *)reader_MMMethodMMM
{
    dispatch_once(&onceToken,^{
        coreReader = [[ExperOfficialatic alloc] init];
    });
    return coreReader;
...
/**
  ny people view sleep 
**/
    self=[super init];
//it to the finish line (literally or figuratively) and can show you how it's done. You'll pick up a bit just by spending time together because you'll be inclined to conform to their patterns of behavior
    if (self)
    {
        SDK_LOG(@"reader init");

/**
  黑夜降临，褪黑素的水平就会升高，暗示身体该睡觉了。光线会阻止身体分泌褪黑素，所以白天 
**/
...
/**
  te success. You can make a bet with a friend that you'll 
**/
    return self;
}
...
/**
  that normally feels like a chore more fun is to combine it with a guilty pleasure. Consider only letting yourself watch your favorite TV show at the gym so you'll start looking forward to workouts. Or only letting yourself drink a mocha latte during study sessions so there is a hook to get you to the 
**/
...
/**
  ve left in your day," he advised. "See your sleep as the amount of time you need in order to set yourself up for a productive tomorrow."
...
**/
                [areaInfo_arry enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (obj) {
//al so you'll feel ashamed if they check back later and find out you haven't followed through.
//
//一个简单的方法是把你的目标告诉一些人，这样，如果他们后来发现你没有坚持下去，你会感到羞愧。
//
//
//
//A steeper penalty than shame, however, is putting cold hard cash on the table, and there is excellent e
                        [self.areaCodeDic addEntriesFromDictionary:obj];
                    }

/**
   30 minutes, get up and reset. Maybe you just need five minutes to get sleepy, or maybe an hour, but don't spend that time awake in bed."

格兰德纳说：“你可以给出的最好的睡眠建议就是醒了就起床，不要醒着躺在床上。无论是刚入夜还是夜已深，如果你躺在床上二三十分钟都睡不 
**/
                }];
            }
        }
...
/**
  h in your judgment, catch the star that twinkles in your heart, and it will lead you to your destiny’s path. Follow that pathway and uncover the sweet sunrises that await you.
...
/**
  sh line (literally or figuratively) and can show you how it's done. You'll pick up a bit just by spending time together because you'll be inclined to conform to their patter 
**/
...
/**
  day, worth delaying to catch up on housework, schoolwork, office work 
**/
-(NSDictionary *)coreConfDic{
    if (!_coreConfDic) {
        _coreConfDic = [self readCoreConfInfo_MMMethodMMM];
//是害怕你……我只是害怕失去你，害怕你会突然消失……
//You don't know how long I've waited for you. And so the lion fell in lov
    }
    return _coreConfDic;
...
/**
  till love you till the end of time.
就算来自不同世界，依旧爱你到地老天荒。
...
/**
   I miss going to work in the morning and knowing that at the end of the day,
...
/**
  s, he said, your sleep rhythms aren't predictable and the body doesn't know how to respond.
...
/**
  stiny is hiding behind them. Accept the fact that not everyone is going to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, and it will lead you to your destiny’s path. Follow that pathway and uncover the sweet sunrises that await you.
...
//e replied, “what if I do this?” He dropp
    NSDictionary * infoDic=[NSDictionary dictionaryWithContentsOfFile:infoPlistPath];
    return infoDic;

/**
  ion. So if a person needed to rise at 7 am each day, backtiming eight hours would require a bedtime of 11 pm.

美国疾病控制与预防中心指出，大多数成人需要7到8小时睡眠时间才能得到充分休息。因此如果一个人每天要在早上7点起床，倒推8个小时睡眠时间应该是晚上11点。

"Now you know when you have to stop and get ready to go to bed whether you're done or not," Grander said. "The problem is we don't stop, and we don't disconnect. And that 
**/
}
...
If you deviate at all from your New Year's resolution, your instinct may be to declare yourself a failure and throw in the towel. Researche 
**/
            return infoDic;
        }

/**
   the beginning of the night or the middle of the night, 
**/
    }

/**
  ell in love with the lamb. What a stupid lamb. What a sick, masochistic lion.
...
/**
  way the sun shines at your face and the way the cold breeze of the morning surprises us.

我想念阳光照在你的脸上，早晨的寒风让我们感到惊讶。

I miss the morning kisses and the morning 
**/
    return nil;
}
...
Happily, there is a way to dodge this fate. By setting tough goals (like a 10 pm bedtime every night) but giving yourself one or two get-out-of-jail-free cards each week, you can get better results t 
**/
        return strconfig;
    }
    return @"";
...
/**
   release of this hormone needs to occur at regular times, said clinical psychologist and sleep expert Michael Grandner. So if your bedtime and wake up time change from day to day or on weekends, he said, your sleep rhythms aren't predictable and the body doesn't know how to respond.

临床心理学家、睡眠专家迈克尔·格兰德纳指出，为了让身体正常分泌褪黑素，你必须规律作息。如果你的上床和起床时间每天都不一样或者一 
**/
}
...
//sleepy and when you wake up. As night approac
-(NSString *)getLocalizedStringForKey_MMMethodMMM:(NSString *)key
{
// whether you enjoy the act of goal pursuit.
//
...
//
//
//If it's not fun to exercise or study, you're unlikely to keep at it. But if you get pleasure from your workouts or study sessions, research has found you'll persis
    return NSLocalizedStringFromTableInBundle(key, _m_stringsName, _m_stringsBundle, nil);
}
- (void)setBundleInfo_MMMethodMMM {

/**
  s place where you toss and turn and wake up tired.
...
/**
  rumpled, and ground into the dirt by the decisions we make and the circumstances that come our way. We feel as though we are worthless；but no matter what happened or what will happen, you will never lose your value."
...
/**
  eping as the final thing they have to do in a jam-packed day, worth delaying to catch up on housework, schoolwork, office work or the latest binge-worthy television series.
...
/**
  s of data," Grandner said. In fact, he said this tip is so powerful that when used in his sleep clinic it "can even 
**/
...
//own that adding a cue to your plan helps you remember when to act. Be sure to detail when and where you'll follow through.
//
...
{
    if ([self isMoreLanguage_MMMethodMMM]) {
        return [DrapfoldWife getServerLanguage_MMMethodMMM];
    }
    if ([NautindustryitSimplyesque isNotEmpty_MMMethodMMM:[self getStringForKey_MMMethodMMM:@"gameLanguage"]]) {
// nobody dies.
//童年是没有生老病死的国度。
...
//orm an association in your brain that can lead to chronic insomnia, Grandner explained. Instead of being a restful spot where you peacefully fall asleep, your bed becomes an anxious place where you toss and turn and wake up tired.
//
//为什么这条法则如此重要呢？格兰德纳解释道，因为醒着躺在床上会在大脑中形成关联，从而导致长期失眠。这样一来，床就不再是你安眠的舒适窝，而成为你辗转反侧心生焦虑并在疲惫中醒
    return @"zh_TW";
}
...
//图片
//Bella, the only reason I left was because I thought 
{
...
/**
   your destiny is hiding behind them. Accept the fact that not everyone is going to approve of the choices you’ve made, have faith in your judgment, catch the star that twinkles in your heart, and it will lead you to your destiny’s p 
**/
{
    return [self getStringForKey_MMMethodMMM:@"sdk_url_cdn"];
}
//ting you ready to greet the day.
//
//褪黑素是身体分
...
As long as i live, my feeling for you wil 
**/
}
...
One way to make pursuing a goal that normally feels like a chore more fun is to c 
**/
...
//
//
//Spending time around high achievers can boost your own performance. If your New Year's resolution is to run a marathon or write a book, you'd be wise to start hanging around friends who'
{
...
If you deviate at all from your New Year's resolution, your instinct may be to declare yourself a failure and throw in the towel. Researchers call this the "what the hell effect." Here's what it looks like: You planned to get to bed early every 
**/
{
...
/**
  acles -- so if you plan to meditate during lunch, you'll be sure to decline a proffered lunch meeti 
**/
...
//
//
//Just as cues tell Broadway stars when to step onto the stage, research has shown that adding a cue to your plan helps you remember when to act. Be sure to detail when and where you'll
...
//
//
//Happily, there is a way to dodge this fate. By setting tough goals (like a 10 pm bedtime every night) but giving yourself one or two get-out-of-jail-free cards each week, you can get be
-(NSString *)getFacebookAppId_MMMethodMMM
{
// I miss all the mornings that I have woken up right
