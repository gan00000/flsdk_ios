...
FUNCTION_NAME="IAP"  #生成库文件的名字的功能描述，如Login， Ad等

DOTA_BUILD_FLODER_NAME="GamaIAP_DotA" #请输入你需要生成的target名字。一定要写正确哦

#######################################################
#时间信息
CUR_TIME=`date "+ %Y-%m-%d %H:%M:%S"`
DATE=`date +%Y%m%d_%H%M%S`
OUTPUT_NAME="$PREFIX_NAME""_""IAP"".a"
...
#
#xcodebuild -workspace PREFIX_NAME −target
#{PREFIX_NAME} -scheme ${PREFIX_NAME} -configuration Release -sdk iphoneos
#
#xcodebuild archive -workspace PREFIX_NAME −scheme
#{PREFIX_NAME} -archivePath /Users/xxxx/Desktop/ipa/PREFIX_NAME.xcarchive
