...
{
    NSString * gamaUUid = [GamaCfUUID getGamaCfUUID];
    return gamaUUid;
}
...
+ (UIViewController *)getCurrentViewController
{
    ...
    if (keyWindow !=  nil && !keyWindow.isKeyWindow) {
        GAMA_FUNCTION_LOG(@"Unable to obtain a key window, marking %@ as keyWindow")
...
+ (UIWindow *)getCurrentWindow
{
    ...
    if (window == nil) {
        GAMA_FUNCTION_LOG(@"Unable to find a valid UIWindow");
    }
    return window;
}
...
{
    if (SDKConReaderGetBool(GAMA_GMAE_mLanguge)) {  //多语言版本增加
        return [GamaFunction getPreferredLanguage]? : @"";
    }
