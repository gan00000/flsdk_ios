...
//    self.valueArray = self.btnsDic.allValues;
    ...
    NSLog(@"getBundleVersion=%@", [ArchaeitiveCivilant getBundleVersion_MMMethodMMM]? : @"");
    NSLog(@"getBundleShortVersionString=%@", [ArchaeitiveCivilant getBundleShortVersionString_MMMethodMMM]? : @"");
    ...
    switch (index) {
        ...
        case 0:
        {
            [[MWSDK share] sdkLoginWithHandler:^(LoginData *loginData) {
                ...
                [StrigLevelule showAlertWithMessage_MMMethodMMM:[NSString stringWithFormat:@"userId:%@, accessToken:%@, timestamp:%@", userId, accessToken, timestamp]];
...
//            com.fzzh.tw.2usd
        ...
        case 5:{
            ...
            [[MWSDK share] shareWithTag:@"#萬靈召喚師" message:@"2022首款卡牌大作【萬靈召喚師】，爆笑來襲！從東方文明到西方文明的羈絆，從神族到魔族的對抗，一段奇妙的神仙冒險之旅就此展開！" url:@"https://share.leyouye.com/aedzj/1.html" successBlock:^(BOOL success, NSDictionary * _Nullable result) {
                ...
                if (success) {
                    [StrigLevelule showAlertWithMessage_MMMethodMMM:@"分享成功"];
                }else{
                    [StrigLevelule showAlertWithMessage_MMMethodMMM:@"分享失败"];
                }
            }];
...
- (void)pay:(NSString *)aProductID {
    ...
    NSString *cpOrderId = [ArchaeitiveCivilant getTimeStamp_MMMethodMMM];//游戏订单ID
    ...
    [[MWSDK share] payWithRoleId:@"22006992996306" roleName:@"哈哈下" roleLevel:@"100" roleVipLevel:@"0" serverCode:@"999" serverName:@"无敌服" productId:aProductID cpOrderId:cpOrderId extra:extra completionHandler:^(SDK_PAY_STATUS status, PayData *mPayData) {
        ...
        switch (status) {
            case SDK_PAY_STATUS_SUCCESS:
                ...
                [StrigLevelule showAlertWithMessage_MMMethodMMM:@"充值成功"];
                break;
                
            case SDK_PAY_STATUS_FAIL:
                [StrigLevelule showAlertWithMessage_MMMethodMMM:@"充值失败"];
                break;
