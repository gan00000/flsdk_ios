    ...
    switch (indexPath.row) {
        ...
        case 1:{//获取用户信息
            [MWSDK gama_getUserProfileWithPartyKind:GamaThirdPartyTypeFacebook completedHandler:^(NSDictionary * _Nullable result, NSError * _Nullable error) {
                if (error) {
                    ...
                    [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"获取用户信息失败:%@",error]];
                    return ;
                }
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"成功获取用户信息:%@",result]];
            }];
        }break;
        case 2:{//获取在玩好友（首页）
            [MWSDK gama_fetchFriendsWithKind:GamaThirdPartyTypeFacebook
                  ...
                  if (error) {
                      NSLog(@"获取用户信息失败:%@",error);
                      [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"获取在玩好友信息失败:%@",error]];
                      return ;
                  }
                  ...
                  [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"获取在玩好友信息失成功：%@",invitableArr]];
            }];
        }break;
        case 3:{//获取在玩好友（下一页）
            [MWSDK gama_fetchFriendsWithKind:GamaThirdPartyTypeFacebook
        ...
        case 5:{//分享链接
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeFacebook param:@{GAMA_PRM_SOCIAL_SHARE_LINK:@"https://developers.facebook.com/"} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"分享成功:%@",result]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"分享失败:%@",error]];
            }];
        }break;
        case 6:{//分享图片
...
//            }];
        ...
        case 9:{//邀请在玩好友
            ...
            [MWSDK gama_inviteFriendsWithKind:GamaThirdPartyTypeFacebook param:@{GAMA_PRM_SOCIAL_INVITE_USERID: lastUserIdAry,GAMA_PRM_SOCIAL_INVITE_MSG:@"hello",GAMA_PRM_SOCIAL_INVITE_TITLE:@"邀请好友"} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"邀请成功:%@",result]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"邀请失败:%@",error]];
            }];
        }break;
        case 10:{//邀请未玩好友
        ...
        case 11:{//line文字分享
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeLine param:@{GAMA_PRM_SOCIAL_SHARE_MSG:@"hello"} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"line文字分享成功：:%@",result]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"line文字分享失败：:%@",error]];
            }];
        }break;
        ...
        case 12:{//line图片分享
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeLine param:@{GAMA_PRM_SOCIAL_SHARE_LOCAL_IMAGE:shareImage} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"line图片分享成功：:%@",result]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"line图片分享失败：:%@",error]];
            }];
        }break;
        ...
        case 13:{//whatsApp文字分享
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeWhatsApp param:@{GAMA_PRM_SOCIAL_SHARE_MSG:@"hello"} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"whatsAapp文字分享成功：:%@",result]];
            }failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"whatsAapp文字分享失败：:%@",error]];
        ...
        case 14:{//whatsApp图片分享
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeWhatsApp param:@{GAMA_PRM_SOCIAL_SHARE_LOCAL_IMAGE:shareImage} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"whatsAapp图片分享成功：:%@",result]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"whatsAapp图片分享失败：:%@",error]];
            }];
        }break;
        case 15:{
            [MWSDK gama_shareWithKind:GamaThirdPartyTypeTwitter param:@{GAMA_PRM_SOCIAL_SHARE_LOCAL_IMAGE:@"Default-568h@2x.png"} success:^(NSDictionary * _Nullable result) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"分享成功"]];
            } failure:^(NSError * _Nullable error) {
                [DreamismAlthougheous showAlertWithMessage:[NSString stringWithFormat:@"分享失败"]];
