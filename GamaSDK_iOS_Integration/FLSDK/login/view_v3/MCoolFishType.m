...
@property(nonatomic, copy)NSString *  tableSdkMore_string;
@property(nonatomic, assign)int  draw_sum;
@property(nonatomic, copy)NSString *  filedImageString;
@property(nonatomic, copy)NSArray *  expressionList;
...
{
   self.tableSdkMore_string = @"invert";

   self.draw_sum = 61;

   self.filedImageString = @"footstep";

   self.expressionList = @[[NSDictionary dictionaryWithObjectsAndKeys:@"attest",@(216), @"motive",@(85), @"bottleneck",@(122), nil]];

   self.levelBtn_arr = [NSArray arrayWithObjects:@"absolve", @"blooming", nil];
