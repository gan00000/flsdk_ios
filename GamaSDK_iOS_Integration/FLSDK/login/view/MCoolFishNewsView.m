...
@implementation MCoolFishNewsView

{
    MCoolFishFiledEncodingView *oldPwdSDKTextFiledView;
...
{
   self.wrapper_size = 847.0;

   self.line_mark = 2735;

   self.window_o = 5766.0;
    ...
    if (self) {
        ...
        mLoginTitleView = [[MCoolFishFiledGestureView alloc] initViewWithTitle_MMMethodMMM:GetString(wwwww_tag_wwwww_text_change_pwd) hander_MMMethodMMM:^(NSInteger) {
        ...
        oldPwdSDKTextFiledView = [[MCoolFishFiledEncodingView alloc] initViewWithType_MMMethodMMM:(SDKTextFiledView_Type_Password_Old)];
        ...
        UIButton *okBtn = [MCoolFishConfig initBtnWithTitleText_MMMethodMMM:GetString(wwwww_tag_wwwww_text_confire_change) fontSize_MMMethodMMM:FS(17) textColor_MMMethodMMM:[UIColor whiteColor] tag_MMMethodMMM:kOkActTag selector:@selector(registerViewBtnAction_MMMethodMMM:) target_MMMethodMMM:self];
...
-(void)changePassword_MMMethodMMM
{
    ...
    if ([MCoolFishSecurityHome isEmpty_MMMethodMMM:oldPwd]) {
    ...
    if (![MCoolFishEventFile validPwd_MMMethodMMM:againPwd]) {
        return;
    }
    ...
    kWeakSelf
    [MCoolFishWithCommon doChangePasswordWithUserName_MMMethodMMM:userName andOldPassword_MMMethodMMM:oldPwd andNewPassword_MMMethodMMM:newPwd otherParamsDic_MMMethodMMM:nil successBlock_MMMethodMMM:^(id responseData) {
