







#import "CalidabilityMemoryaticView.h"
#import "SdkHeader.h"


NS_ASSUME_NONNULL_BEGIN

@interface DesHalfryTextFiled : CalidabilityMemoryaticView

@property (nonatomic,strong) UITextField *inputTextField;
@property (nonatomic, strong) NSMutableArray *afheavyelMirfilmior;//===insert my property===

@property (nonatomic, assign) CGFloat alwaysVagature;//===insert my property===


- (instancetype)initWithTextColor_MMMethodMMM:(UIColor *)textColor fontOfSize_MMMethodMMM:(CGFloat)size placeholder_MMMethodMMM:(NSString*)placeholder placeColor_MMMethodMMM:(UIColor *)placeColor;

@end

NS_ASSUME_NONNULL_END
