...
@interface MCoolFishInterruptView()



@property(nonatomic, copy)NSString *  expressionStr;
@property(nonatomic, copy)NSString *  firebaseScriptClickString;
@property(nonatomic, assign)long  sdkSum;
@property(nonatomic, copy)NSArray *  helperArray;


@end

@implementation MCoolFishInterruptView
{
...
{
   self.expressionStr = @"limbo";

   self.firebaseScriptClickString = @"objectionable";

   self.sdkSum = 6738;

   self.helperArray = [NSArray arrayWithObjects:@(516), @(402), @(712), nil];

   self.password_max = 1381.0;

   self.urlsGestureInterfaceArr = [NSArray arrayWithObjects:@(586), @(569), @(976), nil];

   self.textMax = 3025.0;

   self.service_mark = 6796;
...
- (instancetype)initView_MMMethodMMM
{
    ...
    if (self) {
        ...
        accountLoginBtn = [MCoolFishConfig initBtnWithTitleText_MMMethodMMM:wwwww_tag_wwwww_vigenast_claimistic fontSize_MMMethodMMM:FS(17) textColor_MMMethodMMM:[UIColor whiteColor] tag_MMMethodMMM:kAccountLoginActTag selector:@selector(registerViewBtnAction_MMMethodMMM:) target_MMMethodMMM:self];
        ...
        if (mAccountArray.count > 0){
            ...
            [MCoolFishInterruptView makeAccountFiledViewStatus_MMMethodMMM:currentAccountModel accountView_MMMethodMMM:accountSDKTextFiledView pwdView_MMMethodMMM:passwordSDKTextFiledView];
        }
        ...
        accountSDKTextFiledView.inputTextFieldChange = ^(NSString *msg, NSInteger m, NSDictionary *dic) {
            ...
            if (currentAccountModel && ![currentAccountModel.loginType isEqualToString:LOGIN_TYPE_SELF]) {
                ...
                if (![msg isEqualToString:currentAccountModel.userId]) {
                    ...
                    [MCoolFishInterruptView makeAccountFiledViewStatus_MMMethodMMM:tempAccountModel accountView_MMMethodMMM:accountSDKTextFiledView pwdView_MMMethodMMM:passwordSDKTextFiledView];
    ...
    accountListView.mAccountModelClickHander = ^(BOOL isDelete, AccountModel * _Nullable aModel, NSMutableArray<AccountModel *> *accountDataList) {
        ...
        if (isDelete) {
            ...
            if (accountDataList.count > 0) {
                ...
                [MCoolFishInterruptView makeAccountFiledViewStatus_MMMethodMMM:blockSelf->currentAccountModel accountView_MMMethodMMM:accountSDKTextFiledView pwdView_MMMethodMMM: passwordSDKTextFiledView];
            }else{
