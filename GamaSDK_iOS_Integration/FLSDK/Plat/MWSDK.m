         ...
         annotation:(id)annotation
{
    ...
    [CHMXTTColor application:application openURL:url sourceApplication:sourceApplication annotation:annotation];
    BOOL result = [CHMXTTResp application:application openURL:url sourceApplication:sourceApplication annotation:annotation];
    if (!result) {
        result = [CHMXTTScriptSelect application:application openURL:url sourceApplication:sourceApplication annotation:annotation];
    }
...
- (BOOL)application:(UIApplication *)application openURL:(NSURL *)url options:(NSDictionary *)options
{
    ...
    [CHMXTTColor application:application openURL:url options:options];
    BOOL result = [CHMXTTResp application:application openURL:url options:options];
    if (!result) {
        result = [CHMXTTScriptSelect application:application openURL:url options:options];
    }
    return result;
}
...
- (void)sdkLoginWithHandlerForInner
{
    ...
    CHMXTTPhoneController *controller = [[CHMXTTPhoneController alloc] initWithPageType_MMMethodMMM:(SDKPage_Login)];
    ...
    [[CHMXTTBlock getCurrentViewController_MMMethodMMM] presentViewController: controller animated:NO completion:^{
...
-(void)showNoticeView
{
    CHMXTTServiceInterrupt *mNoticeViewV2 = [[CHMXTTServiceInterrupt alloc] initWithCompleter_MMMethodMMM:^{
        
        [self sdkLoginWithHandlerForInner];
    }];
    
    UIView *superView = appTopViewController.view;
    UIView *bgV = [[CHMXTTLoggerBundleView alloc] init];
                   ...
                   serverName:(NSString *)serverName
{
    ...
    // 对必要参数进行检查
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:roleId] || [CHMXTTImplGama isEmpty_MMMethodMMM:roleName] ||
        [CHMXTTImplGama isEmpty_MMMethodMMM:roleLevel] ||
        [CHMXTTImplGama isEmpty_MMMethodMMM:serverCode])
    {
        ...
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"角色重要信息为空,请检查参数中 roleId roleName roleLevel serverCode是否有值"];
        return;
    }
    ...
    completionHandler:(SDKPayBlock) handler
{
    ...
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:productId]) {
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"productId must be not empty"];
        return;
    }
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:roleId]) {
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"roleId must be not empty"];
        return;
    }
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:serverCode]) {
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"serverCode must be not empty"];
        return;
    }
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:cpOrderId]) {
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"cpOrderId must be not empty"];
        return;
    }
    ...
    if (!accountModel || !accountModel.userId) {
        [CHMXTTPrivateOrder showAlertWithMessage_MMMethodMMM:@"error:請重新登入遊戲進行充值"];
        self.isPaying = NO;
        return;
    }
    
    [[CHMXTTBind shareManager_MMMethodMMM] startPayWithProductId_MMMethodMMM:productId cpOrderId_MMMethodMMM:cpOrderId extra_MMMethodMMM:extra gameInfo_MMMethodMMM:SDK_DATA.gameUserModel accountModel_MMMethodMMM:accountModel payStatusBlock_MMMethodMMM:^(BOOL success, PayData * _Nullable payData) {
        ...
        if (self.payHandler) {
            if (success) {
                ...
                BOOL havePay = [CHMXTTBase _userdefaultGetBoolForKey:SDK_DATA.mLoginResponse.data.userId];
                if (!havePay) {
//                    [CHMXTTEnginePple logWithEventName_MMMethodMMM:AD_EVENT_FIRST_PURCHASE parameters_MMMethodMMM:nil];
                }
                [CHMXTTBase _userdefaultSetBool:YES forKey:SDK_DATA.mLoginResponse.data.userId];
                
                [CHMXTTColor logEventPurchaseValues_MMMethodMMM:payData type_MMMethodMMM:(AdType_All)];
...
- (void)shareWithTag:(NSString *)hashTag message:(NSString *)message url:(NSString *)url successBlock:(ShareBlock)shareBlock{
    ...
    [[CHMXTTResp share] shareWithTag_MMMethodMMM:hashTag message_MMMethodMMM:message url_MMMethodMMM:url presentingViewController:appTopViewController successBlock_MMMethodMMM:^(NSString *msg, NSInteger m, NSDictionary *dic) {
...
-(void)showBindPhoneViewWithBlock:(MWBlock) mBlock
{
    ...
    if (!is_Version2) {
        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_meraceous_nugaciular];
        return;
    }
    CHMXTTNews *mBindPhoneViewV2 = [[CHMXTTNews alloc] init];
...
- (void)showUpgradeAccountViewWithBlock:(MWBlock)mBlock
{
    ...
    CHMXTTRequest *mBindAccountViewV2 = [[CHMXTTRequest alloc] initView_MMMethodMMM];
...
- (void)requestVfCodeWithAreaCode:(NSString *)areaCode telephone:(NSString *)telephone Block:(MWBlock)mMWBlock
{
    ...
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:areaCode]) {
        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_text_area_code_not_empty.localx];
        return;
    }
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:telephone]) {
        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_text_phone_not_empty.localx];
        return;
    }
//    if (![CHMXTTRegister validPhone_MMMethodMMM:telephone phoneRegex_MMMethodMMM:mPhoneInfoModel.selectedRegularExpression]) {
//        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_text_phone_not_match.localx];
//        return;
//    }
    
    [CHMXTTChangeTimer requestMobileVfCode_MMMethodMMM:areaCode phoneNumber_MMMethodMMM:telephone email_MMMethodMMM:@"" otherDic_MMMethodMMM:nil successBlock_MMMethodMMM:^(id responseData) {
        
//        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_text_vfcode_has_send.localx];
        if (mMWBlock) {
            mMWBlock(YES, nil);
        }
...
//    }
    ...
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:vfCode]) {
        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_py_msg_vfcode_hint.localx];
        return;
    }
    
    [CHMXTTChangeTimer bindAccountPhone_MMMethodMMM:areaCode phoneNumber_MMMethodMMM:telephone vCode_MMMethodMMM:vfCode otherDic_MMMethodMMM:nil successBlock_MMMethodMMM:^(id responseData) {
        
//        [CHMXTTRegister toastMsg_MMMethodMMM: wwwww_tag_wwwww_text_phone_bind_success.localx];
...
{
    if (![CHMXTTRegister validUserName_MMMethodMMM:account]) {
        return;
    }


    if (![CHMXTTRegister validPwd_MMMethodMMM:password]) {
        return;
    }
...
//        [CHMXTTRegister toastMsg_MMMethodMMM:GetString(wwwww_tag_wwwww_text_account_bind_success2)];
        ...
        [[CHMXTTEfault share] saveAccountModel_MMMethodMMM:cc.data];
...
- (void)openCs
{
    ...
    if ([CHMXTTImplGama isEmpty_MMMethodMMM:csurl]) {
        SDK_LOG(@"客服地址错误 csurl=%@",csurl);
        return;
    }
    NSString *resultURL = [CHMXTTChangeTimer createSdkUrl_MMMethodMMM:csurl];
    [CHMXTTMainController webViewControllerPresentingWithURLRequest_MMMethodMMM:[NSURLRequest requestWithURL:[NSURL URLWithString:resultURL]] layoutHandler_MMMethodMMM:nil animation_MMMethodMMM:NO animationStyle_MMMethodMMM:UIModalTransitionStyleCoverVertical];
}
...
//        content.badge = @(1);
        ...
        if ([CHMXTTImplGama isEmpty_MMMethodMMM:notifyId]) {
            notifyId = [CHMXTTBlock getMD5StrFromString_MMMethodMMM:title];
        }
