...
@interface PhaeyounghoodFutil()


@end

@implementation PhaeyounghoodFutil

static PhaeyounghoodFutil * configUtil;
...
+ (PhaeyounghoodFutil *)share
{
    ...
    dispatch_once(&onceToken,^{
        configUtil = [[PhaeyounghoodFutil alloc] init];
    });
    return configUtil;
}
...
-(void)saveAccountModel_MMMethodMMM:(AccountModel*) mAccountModel{
    ...
    mAccountModel.lastLoginTime = [BreviVincy getTimeStamp_MMMethodMMM];
    NSArray *mAccountArray = [self getAccountModels_MMMethodMMM];
...
-(void)saveGameUserInfo_MMMethodMMM:(LoginResponse *)loginResopnse
{
    ...
    OffousClaimial *gameUserModel = [[OffousClaimial alloc] init];
...
-(OffousClaimial*)getGameUserInfo_MMMethodMMM:(NSString *)userId
{
    ...
    if(userInfoJson){
        OffousClaimial *gameUserModel = [OffousClaimial yy_modelWithJSON:userInfoJson];
