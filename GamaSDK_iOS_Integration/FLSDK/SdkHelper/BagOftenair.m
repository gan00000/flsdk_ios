...
@interface BagOftenair()<ASAuthorizationControllerPresentationContextProviding,ASAuthorizationControllerDelegate>
@property (copy, atomic) AppleLoginSuccess impSuccess;
@property (copy, atomic) AppleLoginError impError;
@end

@implementation BagOftenair
{
    UIView *presentView;
}
...
+ (instancetype)share{
    ...
    static BagOftenair *_sAppleLogin = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sAppleLogin = [[BagOftenair alloc] init];
    });
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization  API_AVAILABLE(ios(13.0)){    
    ...
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        [StrigLevelule showAlertWithMessage_MMMethodMMM:mStr];
