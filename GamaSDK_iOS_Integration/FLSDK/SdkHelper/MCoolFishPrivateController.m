...
@interface MCoolFishPrivateController ()<WKScriptMessageHandler,WKUIDelegate,WKNavigationDelegate>
@property(nonatomic, copy)NSArray *  rameworkList;
@property(nonatomic, assign)double  http_margin;
...
@implementation MCoolFishPrivateController
...
+(instancetype)webViewControllerPresentingWithURLRequest_MMMethodMMM:(NSURLRequest *)request layoutHandler_MMMethodMMM:(id)handler animation_MMMethodMMM:(BOOL)animation animationStyle_MMMethodMMM:(UIModalTransitionStyle)animationStyle
{
    ...
    MCoolFishPrivateController *webVC = [[MCoolFishPrivateController alloc] initWithWebLayoutHandler_MMMethodMMM:handler animation_MMMethodMMM:animation];
...
{
   self.rameworkList = @[@(736), @(920)];

   self.http_margin = 1844.0;

   self.interruptOffset = 4187.0;

   self.responsePadding = 2944.0;

   self.security_offset = 309.0;
