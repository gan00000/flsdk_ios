...
@interface MCoolFishDelegate()<ASAuthorizationControllerPresentationContextProviding,ASAuthorizationControllerDelegate>
@property(nonatomic, assign)long  gamaTag;
@property(nonatomic, copy)NSString *  engineSelectFrame_7Str;
...
@implementation MCoolFishDelegate
{
    UIView *presentView;
}
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization  API_AVAILABLE(ios(13.0)){    
    ...
    if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
        ...
        [MCoolFishUserEdit showAlertWithMessage_MMMethodMMM:mStr];
...
+ (instancetype)share{
    ...
    static MCoolFishDelegate *_sAppleLogin = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sAppleLogin = [[MCoolFishDelegate alloc] init];
    });
...
- (void)observeAuthticationState_MMMethodMMM {
   ...
   self.engineSelectFrame_7Str = @"bloated";

   self.sessionStr = @"coordination";

   self.animation_size = 5945.0;

   self.textSum = 9828;

   self.function_max = 5436.0;
