...
@implementation ActurnessKnowledgeesque

+ (instancetype)sharedSdkData_MMMethodMMM
{
    static ActurnessKnowledgeesque * pfData = nil;
    static dispatch_once_t once_dispatch;
    dispatch_once(&once_dispatch, ^{
        pfData = [[ActurnessKnowledgeesque alloc] initSDKDATA_MMMethodMMM];
    });
    return pfData;
}
...
- (void)initData_MMMethodMMM
{
    ...
    SocialCheiropolitics *mGameUserModel = [[SocialCheiropolitics alloc] init];
