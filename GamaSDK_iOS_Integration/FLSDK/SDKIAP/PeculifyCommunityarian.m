...
@interface PeculifyCommunityarian ()<SKProductsRequestDelegate,SKPaymentTransactionObserver>
...
@implementation PeculifyCommunityarian
...
- (void)requestPament_MMMethodMMM:(SKProduct *)p {
    ...
    if ([PeopleitionStudentosity getSystemVersion_MMMethodMMM].intValue>7){
...
-(void)handleTransaction_MMMethodMMM:(SKPaymentTransaction *)transaction
{
    ...
    if ([GeoallyKin isEmpty_MMMethodMMM:transactionId]) {
    ...
    NSString *receiptString = [PeopleitionStudentosity encode:(uint8_t *)receiptData.bytes length:receiptData.length];
    ...
    if (!parameterStr || [@"" isEqualToString:parameterStr])
    {
        ...
        if([GeoallyKin isEmpty_MMMethodMMM:parameterStr]){
...
-(void)startPayWithProductId_MMMethodMMM:(NSString *)productId cpOrderId_MMMethodMMM:(NSString *)cpOrderId extra_MMMethodMMM:(NSString *)extra gameInfo_MMMethodMMM:(HerallySolvfier*)gameUserModel accountModel_MMMethodMMM:(AccountModel*) accountModel payStatusBlock_MMMethodMMM:(PayStatusBlock)tappedBlock
{
    ...
    [TermDiseaseid createOrderWithproductId_MMMethodMMM:productId cpOrderId_MMMethodMMM:cpOrderId extra_MMMethodMMM:extra gameInfo_MMMethodMMM:gameUserModel accountModel_MMMethodMMM:accountModel otherParamsDic_MMMethodMMM:nil successBlock_MMMethodMMM:^(id responseData) {
...
- (void)finishPayWithStatus_MMMethodMMM:(BOOL)status msg_MMMethodMMM:(NSString *)msg
{
    ...
    if (status) {
        ...
        if ([GeoallyKin isNotEmpty_MMMethodMMM:msg]) {
            [RiskHalf showAlertWithMessage_MMMethodMMM:msg];
        }
        if (self.tappedBlock) {
